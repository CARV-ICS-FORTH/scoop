...
#define BLOCK_SIZE 60
    ...
    real_t *arg1, *arg2;
    int stride1, stride2, asize3;
    int elems, elemsz;
    elems = BLOCK_SIZE;
    elemsz = BLOCK_SIZE*sizeof(real_t);
    stride1 = NROWS*sizeof(real_t);
    stride2 = NCOLS*sizeof(real_t);
    asize3 = 4*sizeof(uint32_t);
    for(k=0; k<NLOOKAT; k++){
	    for(j=0; j<NCOLS; j+=BLOCK_SIZE){
		    for(i=0; i<NROWS; i+=BLOCK_SIZE){
		      arg1 = &G->conc_T[k][j][i];
		      arg2 = &G->conc[k][i][j];
		      #pragma tpc(arg1(out, stride1, elems, elemsz), arg2(in, stride2, elems, elemsz), desc(in, asize3))
		      SPE_transpose ( arg1, arg2, desc);
// 			    tpc_call(
// 					    3, 3,
// 
// 					    &G->conc_T[k][j][i],
