...
void FFT1D(int,int,int,double *,double *,double *,double *,int,double *,int,
...
char **argv;
{
  ...
  */
  ...
  x = (double *) tpc_malloc(2*(N+rootN*pad_length)*sizeof(double)+PAGE_SIZE);;
  trans = (double *) tpc_malloc(2*(N+rootN*pad_length)*sizeof(double)+PAGE_SIZE);;
  umain = (double *) tpc_malloc(2*rootN*sizeof(double));;  
  umain2 = (double *) tpc_malloc(2*(N+rootN*pad_length)*sizeof(double)+PAGE_SIZE);;
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
  ...
  upriv = (double *) tpc_malloc(2*(rootN-1)*sizeof(double));  
  if (upriv == NULL) {
...
struct GlobalMemory *Global;
{
  ...
  m1 = M/2;
  n1 = 1<<m1;  // n1 is the number of (complex doubles) elements per row.

  bytes_per_row = 2*n1*sizeof(double);
  ...
  int asize1, asize3, asize4;
  for (j=MyFirst; j<MyLast; j+=rows_per_task1) {
    arg1 = &x[2*j*(n1+pad_length)];
    arg3 = &umain2[2*j*(n1+pad_length)];
    asize1 = bytes_per_row*rows_per_task1;
    asize3 = bytes_per_row*rows_per_task1;
    asize4 = sizeof(struct FFTsteps23args);
    #pragma tpc(arg1(inout, asize1), upriv(in, bytes_per_row), arg3(in, asize3), s23args(in, asize4))
    FFTsteps23(arg1, upriv, arg3, s23args);
//     tpc_call( 0, 4,
// 	&x[2*j*(n1+pad_length)], , TPC_INOUT_ARG,
// 	upriv, bytes_per_row, TPC_IN_ARG,
// 	&umain2[2*j*(n1+pad_length)], , TPC_IN_ARG,
// 	s23args, , TPC_IN_ARG );
  }
  tpc_wait_all();
  ...
  for (j=MyFirst; j<MyLast; j+=rows_per_task2) {
    ...
    asize3 = sizeof(struct FFTsteps23args);
    #pragma tpc(arg1(inout, asize1), upriv(in, bytes_per_row), s5args(in, asize3))
    FFTsteps5(arg1, upriv, arg3, s5args);
//     tpc_call( 1, 3,
