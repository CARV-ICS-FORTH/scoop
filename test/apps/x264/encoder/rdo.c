...
#define x264_macroblock_write_cavlc_tpc  static x264_macroblock_size_cavlc_tpc
#include "cavlc.c"
...
/* Sum the cached SATDs to avoid repeating them. */
int sum_satd_tpc( x264_t *h, int pixel, int x, int y, x264_mb_t *mb )
{
    int satd = 0;
    int min_x = x>>2;
    int min_y = y>>2;
...
static inline int sum_sa8d_tpc( x264_t *h, int pixel, int x, int y, x264_mb_t *mb )
{
    int sa8d = 0;
    int min_x = x>>3;
    int min_y = y>>3;
    ...
    int max_y = (y>>3) + (x264_pixel_size[pixel].h>>3);
    if( pixel == PIXEL_16x16 )
        return mb->pic.fenc_sa8d_sum;
    for( y = min_y; y < max_y; y++ )
        for( x = min_x; x < max_x; x++ )
            sa8d += mb->pic.fenc_sa8d[y][x];
    return sa8d;
}
...
static inline int ssd_plane_tpc( x264_t *h, int size, int p, int x, int y, x264_mb_t *mb )
{
    DECLARE_ALIGNED_16(static uint8_t zero[16]);
    int satd = 0;
    ...
    if( p == 0 && mb->i_psy_rd )
    {
        ...
        {
            uint64_t acs = h->pixf.hadamard_ac[size]( fdec, FDEC_STRIDE );
            satd = abs((int32_t)acs - sum_satd_tpc( h, size, x, y, mb ))
                 + abs((int32_t)(acs>>32) - sum_sa8d_tpc( h, size, x, y, mb ));
            satd >>= 1;
        }
        else
        {
            int dc = h->pixf.sad[size]( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
            satd = abs(h->pixf.satd[size]( fdec, FDEC_STRIDE, zero, 0 ) - dc - sum_satd_tpc( h, size, x, y, mb ));
        }
...
static inline int ssd_mb_tpc( x264_t *h, x264_mb_t *mb )
{
    return ssd_plane_tpc(h, PIXEL_16x16, 0, 0, 0, mb)
         + ssd_plane_tpc(h, PIXEL_8x8,   1, 0, 0, mb)
         + ssd_plane_tpc(h, PIXEL_8x8,   2, 0, 0, mb);
}


static int x264_rd_cost_mb_tpc( x264_t *h, int i_lambda2, x264_mb_t *mb, x264_dct_t *dct )
{
...
static uint64_t x264_rd_cost_i8x8_chroma_tpc( x264_t *h, int i_lambda2, int i_mode, int b_dct, x264_mb_t *mb, x264_dct_t *dct )
{
    uint64_t i_ssd, i_bits;

    if( b_dct )
        x264_mb_encode_8x8_chroma_tpc( h, 0, mb->i_chroma_qp, mb, dct );
    i_ssd = ssd_plane_tpc( h, PIXEL_8x8, 1, 0, 0, mb ) +
            ssd_plane_tpc( h, PIXEL_8x8, 2, 0, 0, mb );
    ...
    if( h->param.b_cabac )
    {
        x264_cabac_t cabac_tmp;
        COPY_CABAC;
        x264_i8x8_chroma_size_cabac_tpc( h, &cabac_tmp, mb, dct );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
    {
