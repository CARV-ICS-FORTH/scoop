...
static void refine_subpel_tpc( x264_t *h, x264_me_t *m, int hpel_iters, int qpel_iters, int *p_halfpel_thresh, int b_refine_qpel, x264_mb_t *mb );

#define BITS_MVD( mx, my )\
   ( (unsigned) (p_cost_mvx[(mx)<<2] + (unsigned) p_cost_mvy[(my)<<2]) )
...
void x264_me_search_ref_tpc( x264_t *h, x264_me_t *m, int16_t (*mvc)[2], int i_mvc, int *p_halfpel_thresh, x264_mb_t *mb )
{
...
{
    int cost = h->pixf.fpelcmp[i_pixel]( m->p_fenc[0], FENC_STRIDE,
                   &p_fref[0], m->i_stride[0] )  + BITS_MVD(0,0);


    printf("PPU: cost: %d  mvd_cost: %d\n", cost ,BITS_MVD(0,0) );
    COPY3_IF_LT( bcost, cost, bmx, 0, bmy, 0 );
}
#else
    COST_MV( 0, 0 );
#endif
...
#if 0
int ii, jj;
char *tmp11=&p_fref[0];// + m->i_stride[0] ;
printf("\nPPU:\n");
for (jj=0; jj<16; jj++ ){
for ( ii=0 ;ii<16; ii++ )
	printf("%x",tmp11[ii]);
 printf("\n");
 tmp11=tmp11 + m->i_stride[0] ;
}
...
#define SAD_THRESH(v) ( bcost < ( v >> x264_pixel_size_shift[i_pixel] ) )
    ...
    case X264_ME_TESA:
        {
            ...
            int xn;
            uint16_t *cost_fpel_mvx = x264_cost_mv_fpel[mb->i_qp][-m->mvp[0]&3] + (-m->mvp[0]>>2);
            ...
            if( mb->i_me_method == X264_ME_TESA )
            {
 ...
 //   printf("PPU cost:%d mx:%d my:%d\n",bcost,bmx,bmy);
    ...
    if( mb->i_subpel_refine >= 2 )
    {
        ...
        refine_subpel_tpc( h, m, hpel, qpel, p_halfpel_thresh, 0, mb );
    }
...
void x264_me_refine_qpel_tpc( x264_t *h, x264_me_t *m, x264_mb_t *mb )
{
    ...
    refine_subpel_tpc( h, m, hpel, qpel, NULL, 1, mb );
}
...
int x264_me_refine_bidir_tpc( x264_t *h, x264_me_t *m0, x264_me_t *m1, int i_weight, x264_mb_t *mb )
{
    const int i_pixel = m0->i_pixel;
    const int bw = x264_pixel_size[i_pixel].w;
    const int bh = x264_pixel_size[i_pixel].h;
    const int16_t *p_cost_m0x = m0->p_cost_mv - x264_clip3( m0->mvp[0], mb->mv_min_spel[0], mb->mv_max_spel[0] );
    const int16_t *p_cost_m0y = m0->p_cost_mv - x264_clip3( m0->mvp[1], mb->mv_min_spel[0], mb->mv_max_spel[0] );
    const int16_t *p_cost_m1x = m1->p_cost_mv - x264_clip3( m1->mvp[0], mb->mv_min_spel[0], mb->mv_max_spel[0] );
    const int16_t *p_cost_m1y = m1->p_cost_mv - x264_clip3( m1->mvp[1], mb->mv_min_spel[0], mb->mv_max_spel[0] );
    ...
    if( bm0y > mb->mv_max_spel[1] - 8 ||
        bm1y > mb->mv_max_spel[1] - 8 )
        return bcost;
...
void x264_me_refine_qpel_rd_tpc( x264_t *h, x264_me_t *m, int i_lambda2, int i8, x264_mb_t *mb )
{
