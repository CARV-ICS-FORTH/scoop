...
static void inline x264_init_mb_table(x264_t *h){
...

      {
          ...
          uint8_t *pool = x264_malloc( (i_mbs+16)*sizeof(uint8_t)  + (i_mbs+16)*sizeof(uint8_t) + 2*h->sps->i_mb_height*sizeof(uint8_t) );
          memset(pool, 0x00,  (i_mbs+4)*sizeof(uint8_t)  + (i_mbs+4)*sizeof(uint8_t) + 2*h->sps->i_mb_height*sizeof(uint8_t) );
  	  h->mb_done_map = pool;
  	  h->mb_call_map = pool +  (i_mbs+4)*sizeof(uint8_t) ;
  	  h->mb_filter_map = pool +  2*(i_mbs+4)*sizeof(uint8_t)  ;
	...
	for (i=0;i<X264_START_MBS_THR*i_mbs;i++){

//#ifdef HPAGE 
#if 0
	 	void *alloc = allocate_from_huge(  
...
*/
void recycle( x264_t *h, x264_mb_t *mb){
    ...
    int i_list;
    {
        if( mb->i_neighbour & MB_TOPLEFT ){
            ...
            { 		
                mb = h->mb_table[ i_mb_x -1 + max_row*(i_mb_y-1) ];
                ...
                int i_mb_xy_bottom_right = i_mb_x + 1 + i_mb_y*max_row ;
                /* calc xy of the right mb */
                int i_mb_xy_right =  i_mb_x + 1 + (i_mb_y+1)*max_row;


                if (mb->i_mb_xy!=0)
                    if (  ( h->mb_done_map[i_mb_xy_bottom_right]) &&
                             h->mb_done_map[i_mb_xy_right])
                    {
...
void static inline  *alloc_mb(x264_t *h,int mb_xy){
#ifdef STAT
   uint64_t   tmp_time1 = __mftb();
#endif
	...
	int idx ;
	if ( h->mb_table[mb_xy]  ){
		PREFETCH(h->mb_table[mb_xy]);
		x264_mb_t *mb = h->mb_table[mb_xy] ;
...
//#ifdef HPAGE
#if 0
		const void *alloc = allocate_from_huge(  
						ALIGN( sizeof(x264_mb_t),ALIGN_SZ ) + 
						ALIGN( sizeof(x264_dct_t),ALIGN_SZ)  + 
						ALIGN( sizeof(x264_data_t),ALIGN_SZ)  
					);
		...
		h->dct_table[mb_xy] = alloc +   ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) ;
		h->x264_data_table[mb_xy] =  alloc +   (ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) + ALIGN ( sizeof(x264_dct_t),ALIGN_SZ)) ;
		...
		data-> b_mbaff           =  h->sh.b_mbaff;
         ...
         h->mb_table[mb_xy]->pic.i_fref[0] = h->i_ref0;
         h->mb_table[mb_xy]->pic.i_fref[1] = h->i_ref1;
#ifdef STAT
     stats[h->sh.i_type][ALLOCATOR] += ( __mftb() - tmp_time1 );
#endif
	return;
...
#define ENTROPY_PPE_PARALLEL(){									\
	if ( h->mb_done_map[(h->entropy_idx)]  ){		\
		x264_mb_t *mb;								\
...
#define INIT_DATA()						\
{								\
    memset(h->mb_filter_map, 0x0,  1*h->sps->i_mb_height * sizeof( uint8_t)/8 );\
							\
}
...
#define CP_RANGE_LEFT(left)					\
{								\
	mb->mv_max_spel[1] = (left)->mv_max_spel[1];		\
								\
	/* Fullpel MV range for motion search */		\
	mb->mv_min_fpel[1]= (left)->mv_min_fpel[1];		\
	mb->mv_max_fpel[1]= (left)->mv_max_fpel[1];		\
                                                                \
        mb->i_last_qp = (left)->i_last_qp;                      \
        mb->chroma_pred_mode = (left)->chroma_pred_mode;        \
        mb->i_qp = (left)->i_qp;				\
        mb->i_chroma_qp = (left)->i_chroma_qp ;			\
        mb-> i_last_qp = (left)->i_last_qp;			\
        mb-> i_last_qp = h->mb.i_last_qp;			\
}		
...
#define FILTER(){						\
	if (   	k==(max_row-1)  && 				\
		...
		{	\
	    		int b_hpel = h->fdec->b_kept_as_ref; \
			int b_end = l == h->sps->i_mb_height;\
			int b_deblock = !h->sh.i_disable_deblocking_filter_idc;\
			int min_y = l - (1 << h->sh.b_mbaff);\
			int max_y = b_end ? h->sps->i_mb_height : l;\
			b_deblock &= b_hpel || h->param.psz_dump_yuv;\
...
if (!h->mb_filter_map){
    ...
    /* Bitmaps for tasks */
    h->mb_done_map = h->thread[0]->mb_done_map ;
    h->mb_call_map = h->thread[0]->mb_call_map  ;
    h->mb_filter_map = h->thread[0]->mb_filter_map ;
...
INIT_DATA()
    ...
    int max_col =  h->sh.i_last_mb/h->sps->i_mb_width; 
    int max_row =  h->sps->i_mb_width;
     ...
     for ( i_mb_y=0; i_mb_y<=max_row+max_col*2; i_mb_y++){
		...
		/* First loop for analyse  */
                for ( k=i_mb_y,l=0; k>=0 ; k-=2,l++){
					...
					entropy_spe(h);
					if ( h->mb_call_map[mb_xy]  ) 	 continue;

					 h->mb_call_map[mb_x]=1;

#ifdef MEMORY_MANAGE
					/* Get next MB  */
					alloc_mb(h,mb_xy); 
     					if (mb_xy==0) old_mb=h->mb_table[0];
#endif
				        mb = h->mb_table[mb_xy];
					dct = h->dct_table[mb_xy];

					/* Copy some things */
					CP_RANGE_LEFT(old_mb);
		
					/* Load Macroblock Buffers */
        				x264_macroblock_cache_load_tpc( h, k, l, mb );
					entropy_spe(h);
     					x264_macroblock_call_analyse_tpc( h, mb, mb_xy, dct, k, l );
					{
						...
						x264_mb_t *mb; 
						x264_dct_t *dct;
	    					for ( ii_mb_y=i_mb_y-4; (ii_mb_y< i_mb_y+4) && ( ii_mb_y<=max_row+max_col*2)  ; ii_mb_y++)
		               			 for ( ik=ii_mb_y,il=0; ik>=k  ; ik-=2,il++){
                		                	if ( ik<max_row && il<max_col){
                                        			mb_xy = ik + il * (max_row);
								if (  !( h->mb_done_map[mb_xy] ) 
								    && ( h->mb_call_map[mb_xy]  ))
								if ( !(tpc_querry(h->tpc_id_table[ mb_xy ]))){
									...
									if ( !( h->mb_call_map[mb_xy]  )) 
									{
										...
										if( ( h->mb_done_map[mb_xy/8] )   ){
											...
											alloc_mb(h,mb_xy); 
										        ...
										        mb = h->mb_table[mb_xy];
											dct = h->dct_table[mb_xy];
											 h->mb_call_map[mb_xy]=1;
											CP_RANGE_LEFT(  h->mb_table[mb_xy-1] );
									   		x264_macroblock_cache_load_tpc( h, ik+1, il, mb );
											entropy_spe(h);
									...
									if ( il+1 < max_col )			
								   	if ( !( h->mb_call_map[mb_xy] , mb_xy )) 
									{
										entropy_spe(h);
                                        					int mb_xy = ik -3 + (il+1) * (max_row);
										if ( mb_xy>0 && ( (ik-3)>0  )   )
										if( (( h->mb_done_map[mb_xy] , mb_xy ))   ){
											entropy_spe(h);
                                        						mb_xy = ik -2  + (il+1) * (max_row);
											/* Get next data */
											...
											CP_RANGE_LEFT(  h->mb_table[mb_xy-1] );
											entropy_spe(h);
									   		x264_macroblock_cache_load_tpc( h, ik-2, il+1, mb );
   ...
   uint64_t   tmp_time1 = __mftb();
    ...
    FDEC_FILTER();
    entropy_spe(h);
    x264_macroblock_call_memset(h, h->mb_done_map,  ALIGN(2*(h->mb.i_mb_count+16)*sizeof(uint8_t)/8,16)  );
      ...
      i_mb_y = (h->entropy_idx) / h->sps->i_mb_width;
      i_mb_x = (h->entropy_idx) % h->sps->i_mb_width;
#else
      i_mb_y = h->sh.i_first_mb / h->sps->i_mb_width;
      i_mb_x = h->sh.i_first_mb % h->sps->i_mb_width;
#endif
...
int static inline  x264_setup_new_frame(x264_t *h,  x264_picture_t *pic_in, x264_picture_t *pic_out ){
...
printf( " newgraph\n");
 printf( " newcurve pts\n");
    ...
    for (i=500*2;i<h->pos && i< SAMPLES;i++)
	printf("%f %d \n",i*0.079,h->outstanding[i] );
    printf(" linetype solid\n");
    printf(" marktype none\n");
#endif
