...
//#ifdef MAX_CONTEXT
#if 0
    rc = h->rc = x264_malloc(  MAX_CONTEXT * sizeof(x264_ratecontrol_t) );
    memset( rc, 0, 2 * sizeof(x264_ratecontrol_t) );
#else
...
void x264_ratecontrol_mb_tpc( x264_t *h, int bits,x264_mb_t *mb )
{
    x264_ratecontrol_t *rc = h->rc;
    const int y = mb->i_mb_y;
    ...
    if( mb->i_mb_x != h->sps->i_mb_width - 1 || !rc->b_vbv)
        return;
...
static float rate_estimate_qscale( x264_t *h )
{
    ...
    else
    {
        ...
        else /* 1pass ABR */
        {
            ...
            {
#if 0
                int i_frame_done = h->fenc->i_frame + 1 - MAX_CONTEXT;
#else
