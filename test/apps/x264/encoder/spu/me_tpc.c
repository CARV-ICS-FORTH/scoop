...
//   	    fprintf(stderr," >> %d >> %d  >>  %d >> %d \n", min_x, max_x, min_y, max_y );
            ...
            int16_t xs_buf[64];
            int16_t *xs = width<=64 ? xs_buf : malloc( (width+15)*sizeof(int16_t) );
            int xn;
            /// TODO: FIXME
            uint16_t *cost_fpel_mvx = x264_cost_mv_fpel[mb->i_qp][-m->mvp[0]&3] + (-m->mvp[0]>>2);

            sad_x4f[sad_size]( zero, m->p_fenc[0], m->p_fenc[0]+delta,
            ...
            if( mb->i_me_method == X264_ME_TESA )
            {
                ...
                mvsad_t *mvsads = malloc( width*(max_y-min_y+1)*sizeof(mvsad_t) );
                int nmvsad = 0, limit;
                int sad_thresh = i_me_range <= 16 ? 10 : i_me_range <= 24 ? 11 : 12;
                int bsad = sadf[i_pixel]( m->p_fenc[0], FENC_STRIDE, p_fref+bmy*stride+bmx, stride )
                         + BITS_MVD( bmx, bmy );
                for( my = min_y; my <= max_y; my++ )
                {
                    ...
                    if( bsad <= ycost )
                        continue;
                    bsad -= ycost;
                    xn = adsf[i_pixel]( enc_dc, sums_base + min_x + my * stride, delta,
                                               cost_fpel_mvx+min_x, xs, width, bsad*17/16 );
                    for( i=0; i<xn-2; i+=3 )
                    {
                        uint8_t *ref = p_fref+min_x+my*stride;
                        int sads[3];
                        sad_x3f[i_pixel]( m->p_fenc[0], ref+xs[i], ref+xs[i+1], ref+xs[i+2], stride, sads );
                        for( j=0; j<3; j++ )
                        {
                    ...
                    {
                        int mx = min_x+xs[i];
                        int sad = sadf[i_pixel]( m->p_fenc[0], FENC_STRIDE, p_fref+mx+my*stride, stride )
                                + cost_fpel_mvx[xs[i]];
                        if( sad < bsad*sad_thresh>>3 )
                        {
                            COPY1_IF_LT( bsad, sad );
...
static void refine_subpel_tpc( x264_data_t *data, x264_me_t *m, int hpel_iters, int qpel_iters, int *p_halfpel_thresh, int b_refine_qpel, x264_mb_t *mb )
{
    ...
    for( i = hpel_iters; i > 0; i-- )
    {
        ...
        src0 = get_ref( pix[0], &stride, m->p_fref, m->i_stride[0], omx, omy-2, bw, bh+1 );
        src2 = get_ref( pix[1], &stride, m->p_fref, m->i_stride[0], omx-2, omy, bw+4, bh );
        src1 = src0 + stride;
        src3 = src2 + 1;
        fpelcmp_x4f[i_pixel]( m->p_fenc[0], src0, src1, src2, src3, stride, costs );
    ...
    /**
     *  Here we test the range however only the max is tested
     *  TODO: We must add more info for the others
     */
...
int x264_me_refine_bidir_tpc( x264_data_t *h, x264_me_t *m0, x264_me_t *m1, int i_weight, x264_mb_t *mb )
{
