...
#ifdef STAT
uint64_t stats[MAX_FRAME_TYPE][MAX_STAT]={{0}};
uint64_t stats2[MAX_STAT]={{0}};
uint64_t en_stats[MAX_FRAME_TYPE][MAX_STAT]={{0}};
#endif
...
#ifdef TPC
x264_frame_t *Gfenc_next;
x264_picture_t *Gpics[2];
...
#define CHECK_BIT(var, pos) test_bit( (pos), &(var))
#define SET_BIT(var, pos) __set_bit( (pos), &(var) )
...
static int x264_validate_parameters( x264_t *h )
{
    ...
    if( h->param.analyse.i_me_method < X264_ME_DIA ||
        h->param.analyse.i_me_method > X264_ME_OLD_HEX )
...
LOCK_VAR memory_lock;
#endif
/****************************************************************************
 * x264 init and destroy mb_table & x264_data_table & x264_filter_data_table & dct_table & analysis_table:
 ****************************************************************************/
#define ALIGN(x,a) (((x)+((a)-1))&~((a)-1))

static void inline x264_init_mb_table(x264_t *h){

      int i, ii, j;
      int i_mbs =  h->mb.i_mb_count;

      memset(&h->tpc_id_table     , 0x0, X264_MAX_MBS * sizeof( int) );
      memset(h->mb_table     , 0x0, i_mbs * sizeof( x264_mb_t *) );
      memset(h->x264_data_table, 0x0, i_mbs * sizeof( x264_data_t *) );
      memset(h->dct_table, 0x0, i_mbs * sizeof( x264_dct_t  *) );

#ifdef HAVE_PTHREAD2
      //h->mb_encode_mutex = x264_malloc( sizeof ( *h->mb_encode_mutex));
      //LOCK_INIT( &h->mb_encode_mutex, NULL );  
#endif
#ifdef ENTROPY_THREAD
      LOCK_INIT( &h->entropy, NULL );  
#endif
      for (i=0;i< h->sps->i_mb_height;i++){
	      h->x264_filter_data_table_luma[i] = x264_malloc( sizeof(x264_filter_data_t));
	      h->x264_filter_data_table_chroma[i] = x264_malloc( sizeof(x264_filter_data_t));
	      h->x264_filter_data_table_chroma[i]->i_mb_width =  h->x264_filter_data_table_luma[i]->i_mb_width = h->sps->i_mb_width;
	      h->x264_filter_data_table_chroma[i]->i_chroma_qp_offset = h->x264_filter_data_table_luma[i]->i_chroma_qp_offset =  h->param.analyse.i_chroma_qp_offset;
	      h->x264_filter_data_table_chroma[i]->i_mb_stride = h->x264_filter_data_table_luma[i]->i_mb_stride =  h->mb.i_mb_stride;
      }


      {
          //uint8_t *pool = x264_malloc( (i_mbs+16)*sizeof(uint8_t)/8  + (i_mbs+16)*sizeof(uint8_t)/8 + 2*h->sps->i_mb_height*sizeof(uint8_t)/8 );
#ifdef HPAGE
          uint8_t *pool = allocate_from_huge( (i_mbs+8)*sizeof(uint8_t)   );
#else
          uint8_t *pool = x264_malloc( (i_mbs+8)*sizeof(uint8_t)   );
#endif
          memset(pool, 0x00,  (i_mbs+8)*sizeof(uint8_t)  );
  	  h->mb_state_map = pool;
...
#ifdef ENTROPY_THREAD 
LOCK_INIT(&memory_lock,NULL);
#endif

	/* head - tail index */
	h->i_pool_head = h->i_pool_tail = 0;

	/* for stats */
	h->i_pool_count = 0;
	/* FIXME: find a better wayt */
	float thr =  X264_START_MBS_THR;
	if (i_mbs > 3500 ) thr += 0.05;
	if (i_mbs > 7500 ) thr += 0.05;
	if (i_mbs > 9000 ) thr += 0.05;
	const int init_mbs = i_mbs*thr;
	for (i=0;i<init_mbs ;i++){

		void *alloc = x264_malloc(  		ALIGN(sizeof(x264_mb_t), ALIGN_SZ)  +
							ALIGN( sizeof(x264_dct_t), ALIGN_SZ) + 
							ALIGN( sizeof(x264_data_t), ALIGN_SZ)
						  );

                h->mem_pool[h->i_pool_head] =  (x264_mb_t * ) (alloc)  ;
                h->i_pool_head++;
                h->i_pool_count++;


                memset(alloc, 0x00, (ALIGN(sizeof(x264_mb_t), ALIGN_SZ)  +
                            ALIGN( sizeof(x264_dct_t), ALIGN_SZ) + 
                            ALIGN( sizeof(x264_data_t), ALIGN_SZ) 
                            ));
                ...
                for( ii=0; ii<2;ii++ )
                {
                    ...
                    for( j=0; j < i_refs; j++ )
                        (mb->mvr[ii][j]) = (h->mb.mvr[ii][j]);
                }


                for( ii=0; ii<=h->param.b_interlaced; ii++ )
                    for( j=0; j<3; j++ )
                    {
                        mb->intra_border_backup[ii][j] = h->mb.intra_border_backup[ii][j];
                        mb->intra_border_backup[ii][j] += 8;
                    }
                ...
                /* x264_data_t struct */ 
		x264_data_t *   data = (x264_data_t *) ( alloc +  ALIGN( sizeof(x264_dct_t), ALIGN_SZ) +  ALIGN( sizeof(x264_mb_t), ALIGN_SZ) 	 );
		data-> b_dct_decimate    =  h->param.analyse.b_dct_decimate;
		data-> i_range           =  h->param.analyse.i_me_range;
		...
		data-> i_trellis         =  h->param.analyse.i_trellis ;

		data-> i_noise_reduction =  h->param.analyse.i_noise_reduction ;
		data-> b_fast_pskip      =  h->param.analyse.b_fast_pskip ;
...
#else
      const int max_col =  h->sps->i_mb_height; 
      const int max_row =  h->sps->i_mb_width;
      int k,l, i_mb_y;
      for ( i_mb_y=0; i_mb_y<=max_row+max_col*2; i_mb_y++){

                /* First loop for analyse  */
                for ( k=i_mb_y,l=0; k>=0 ; k-=2,l++){
                                if ( k<max_row && l<max_col){
					 i =  k + l * (max_row);
					const void *alloc = x264_malloc(  
						ALIGN( sizeof(x264_mb_t), ALIGN_SZ) + 
						ALIGN( sizeof(x264_dct_t), ALIGN_SZ)  + 
						ALIGN( sizeof(x264_data_t), ALIGN_SZ)  
					);
				         h->mb_table[i]  =  (x264_mb_t *) alloc;
				         h->dct_table[i] = (x264_dct_t *) ( alloc + ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) );
				         h->x264_data_table[i] = (x264_data_t *) ( alloc + ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) + ALIGN ( sizeof(x264_dct_t),ALIGN_SZ)  );
		
				         /// MEMSet ?
				         memset(h->mb_table[i], 0x00, sizeof(x264_mb_t) );
				         memset(h->dct_table[i], 0x00, sizeof(x264_dct_t) );
				         memset(h->x264_data_table[i], 0x00, sizeof(x264_data_t) );

				         /* Now init the macroblocks */
				         h->mb_table[i]->i_mb_count = h->sps->i_mb_width * h->sps->i_mb_height;
					 /* x264_data_t struct */ 
					x264_data_t *data = h->x264_data_table[i];
					data-> b_dct_decimate    =  h->param.analyse.b_dct_decimate;
					...
					data-> i_mb_width        =  h->sps->i_mb_width;
					data-> i_mb_height       =  h->sps->i_mb_height;
					data-> i_direct_mv_pred  =  h->param.analyse.i_direct_mv_pred;
					data-> i_trellis         =  h->param.analyse.i_trellis ;
       ...
       x264_macroblock_cache_init_tpc( h);
       //x264_analysis_table_init(h);
       h->idx = 0;
...
static void inline x264_free_mb_table(x264_t *h){

#if 0
      int i;
      int i_mbs =  ( (h->param.i_width)/16 )* ( ( h->param.i_height)/16 );

     x264_macroblock_cache_end_tpc( h);

     for ( i=0; i< i_mbs; i++){

          x264_free( h->mb_table[i]      );
          x264_free( h->me_data_table[i] );
     }
#endif
...
#ifdef MEMORY_MANAGE
static inline void x264_reference_build_list_tpc( x264_t *h)
{
...
static inline void x264_reference_build_list_tpc( x264_t *h)
{
      ...
      for ( i=0; i< i_mbs; i++){
         h->mb_table[i]->pic.i_fref[0] = h->i_ref0;
         h->mb_table[i]->pic.i_fref[1] = h->i_ref1;
      }
...
static void x264_fdec_copy_intra( x264_t *h, int mb_y, x264_mb_t *mb ){
...
//#ifdef TPC
    ...
    if( !b_end )
    {
        ...
        for( j=0; j<=h->sh.b_mbaff; j++ ){
            x264_macroblock_call_copy_intra( h, j,  mb_y, mb  );
    ...
    if( !b_end )
    {
        ...
        for( j=0; j<=h->sh.b_mbaff; j++ )
            for( i=0; i<3; i++ )
            {
                memcpy( mb->intra_border_backup[j][i],
    ...
    if( b_hpel )
    {
        ...
        //x264_frame_expand_border_tpc( h, h->fdec, min_y, b_end ); 
...
#if 0
static void x264_deblock_row_tpc( x264_t *h, int mb_y, x264_mb_t *mb )
{
    ...
    if( b_deblock )
    {
        ...
        for( y = min_y; y < max_y; y += (1 << h->sh.b_mbaff) )
            x264_frame_deblock_row_tpc( h, y, mb );
...
static void x264_fdec_filter_row_tpc( x264_t *h, int mb_y, x264_mb_t *mb )
{
    /* mb_y is the mb to be encoded next, not the mb to be filtered here */
    int b_hpel = h->fdec->b_kept_as_ref;
    int b_deblock = !h->sh.i_disable_deblocking_filter_idc;
    ...
    if( !b_end )
    {
        ...
        for( j=0; j<=h->sh.b_mbaff; j++ )
            for( i=0; i<3; i++ )
            {
                __builtin_memcpy( mb->intra_border_backup[j][i],
                        h->fdec->plane[i] + ((mb_y*16 >> !!i) + j - 1 - h->sh.b_mbaff) * h->fdec->i_stride[i],
                        h->sps->i_mb_width*16 >> !!i );
            }
    }
...
#ifdef STAT
     stats[h->sh.i_type][FILTER] += ( __mftb() - tmp_time1 );
#endif
    if( b_hpel )
    {
        x264_frame_expand_border( h, h->fdec, min_y, b_end );
...
void termination_dump(x264_t *h,int mb_xy){

int i;
    int max_row =  h->sps->i_mb_width;
int max_mbs = h->mb.i_mb_count;
char * table = x264_malloc(max_mbs );

for (i=0;i<max_mbs;i++)
	table[i] = '0' + h->mb_state_map[i];

table[h->entropy_idx] = 'E';
...
#if 0
{
    /* Now we create mode contexts (x264_t) like threads to overlap frame init */
   ...
   for( i = 0; i < MAX_CONTEXT; i++ )
    ...
    /* Number of SPEs */
    h->spes = param->spes;
    /* Init the table for MB/dct/data */
    x264_init_mb_table( h );

#if 0
   {
        x264_t *lh =  h->thread[1];
	int j;
	for (j=0;j<lh->mb.i_mb_count;j++){
		lh->mb_table[j] =  h->thread[0]->mb_table[j];
		lh->x264_data_table[j] = h->thread[0]->x264_data_table[j];
		lh->dct_table[j] = h->thread[0]->dct_table[j];
	}
   }
#endif
#endif
...
static void x264_fdec_filter_row( x264_t *h, int mb_y )
{
   ...
   uint64_t   tmp_time1 = __mftb();
    ...
    if( !b_end )
    {
        ...
        for( j=0; j<=h->sh.b_mbaff; j++ )
            for( i=0; i<3; i++ )
            {
                ...
                __builtin_memcpy( h->mb.intra_border_backup[j][i],
                        h->fdec->plane[i] + ((mb_y*16 >> !!i) + j - 1 - h->sh.b_mbaff) * h->fdec->i_stride[i],
                        h->sps->i_mb_width*16 >> !!i );
            }
    }
    if( b_deblock )
    {
        int y;
        for( y = min_y; y < max_y; y += (1 << h->sh.b_mbaff) )
            x264_frame_deblock_row_tpc( h, y, &h->mb );
...
#if 0
static void x264_fdec_filter_row2( x264_t *h, int mb_y )
{
#ifdef STAT
   uint64_t   tmp_time1 = __mftb();
#endif
...
static void x264_fdec_filter_row_expand( x264_t *h, int mb_y )
{
#ifdef STAT
   uint64_t   tmp_time1 = __mftb();
#endif
...
static inline void x264_slice_init( x264_t *h, int i_nal_type, int i_global_qp )
{
    ...
    x264_macroblock_slice_init( h );
     ...
     int i;
     for(i=0;i<i_mbs;i++){
        x264_macroblock_slice_init_tpc(h, h->mb_table[i]);
     }
#endif
#endif
...
/*
   Must be called after cache_reload_mv 
*/
void recycle( x264_t *h, int i_mb_xy ){
#ifdef ENTROPY_THREAD
LOCK(&memory_lock);
#endif
    ...
    if (i_mb_xy>0) 
    {
	    h->mem_pool[h->i_pool_head] = h->mb_table[i_mb_xy];
	    h->mb_table[i_mb_xy] = NULL;
	  //  h->dct_table[i_mb_xy] = NULL;
	   // h->x264_data_table[i_mb_xy] = NULL;
	    h->i_pool_head = (h->i_pool_head+1)%X264_MAX_ALLOC ;
    }


#ifdef STAT
    stats[h->sh.i_type][ALLOCATOR] += ( __mftb() - tmp_time1 );
#endif
#endif
...
#else
    int max_row =   h->sps->i_mb_width;
    int max_col =  h->sps->i_mb_height; 
    int i_list;
    {

	    int i_mb_xy = mb->i_mb_xy;
	    /* calc xy of the bottom right mb */
...
void  alloc_mb(x264_t *h,int mb_xy){
#ifdef STAT
   uint64_t   tmp_time1 = __mftb();
#endif
...
LOCK(&memory_lock);
	...
	if ( h->i_pool_head != (h->i_pool_tail+1)){
		h->i_pool_tail = (h->i_pool_tail+1)%X264_MAX_ALLOC ;
		idx = h->i_pool_tail;
		x264_mb_t *mb = h->mb_table[mb_xy] =  h->mem_pool[idx];
		h->dct_table[mb_xy] =  h->mem_pool[idx] +   ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) ;
		h->x264_data_table[mb_xy] =  h->mem_pool[idx] +   (ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) + ALIGN ( sizeof(x264_dct_t),ALIGN_SZ)) ;
		mb->i_mb_xy = mb_xy;


		/* Other */
		int i_mb_y = mb->i_mb_xy/mb->i_mb_stride;
		int i_mb_x = mb->i_mb_xy%mb->i_mb_stride;
		...
		mb->i_mb_y = i_mb_y;
                ...
                mb->ref[0] = h->fdec->ref[0];
                mb->ref[1] = h->fdec->ref[1];
        	mb->pic.i_fref[0] = h->i_ref0;
 	        mb->pic.i_fref[1] = h->i_ref1;
...
#if 0
		// when this called terminates the encoder and print the state of the tables
		if ( h->i_pool_count > h->mb.i_mb_count*0.8 )
			termination_dump(h,mb_xy);
#endif


                x264_mb_t *mb = h->mb_table[mb_xy]  = h->mem_pool[h->i_pool_tail] =  (x264_mb_t *) alloc;
                mb->i_mb_count = h->sps->i_mb_width * h->sps->i_mb_height;
                mb->pic.i_fref[0] = h->i_ref0;
                mb->pic.i_fref[1] = h->i_ref1;

                h->dct_table[mb_xy] = (x264_dct_t *) alloc +   ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) ;
                h->x264_data_table[mb_xy] = (x264_mb_t *) alloc +   (ALIGN ( sizeof(x264_mb_t),ALIGN_SZ) + ALIGN ( sizeof(x264_dct_t),ALIGN_SZ)) ;


                x264_mb_cache_init(h, mb_xy );
                x264_macroblock_slice_init_tpc(h,mb);
                mb->mv[0] = h->fdec->mv[0];
                mb->mv[1] = h->fdec->mv[1];
                mb->ref[0] = h->fdec->ref[0];
                mb->ref[1] = h->fdec->ref[1];
...
#define ENTROPY_PPE_PARALLEL(){									\
	if ( h->mb_state_map[(h->entropy_idx)] == DONE_STATE ){		\
		x264_mb_t *mb;								\
 		mb= h->mb_table[h->entropy_idx];						\
		x264_macroblock_cache_reload_tpc_mv( h, mb->i_mb_x, mb->i_mb_y, mb );	\
		if( IS_SKIP( mb->i_type ) )						\
                      h->i_skip++;								\
                else									\
                {									\
                  {									\
                      bs_write_ue( &h->out.bs, h->i_skip );  /* skip run */		\
                      h->i_skip = 0;							\
                  }									\
  		      x264_dct_t *dct = h->dct_table[h->entropy_idx];			\
                      x264_macroblock_write_cavlc_tpc( h, &h->out.bs, mb, dct ); 	\
            	x264_macroblock_cache_resave_tpc( h, mb ); /* save cache */  		\
       		}									\
		h->entropy_idx++;	 recycle(h,h->entropy_idx-1);					\
	}										\
}
...
if (  (h->mb_state_map[(h->entropy_idx)]==DONE_STATE) ){	
	...
	if( h->entropy_idx){
		if ( !(tpc_querry( h->tpc_id_table[ h->entropy_idx -1]))  ){		
#if VISUALIZE

//   if( h->param.b_visualize )
//	  x264_visualize_show_entropy_tpc( h, h->entropy_idx  );
#endif
			...
			if (!tpc_issue_querry()) { 
				...
				ENTROPY_PPE();
#ifdef STAT
				stats[h->sh.i_type][ENTROPY_PARALLEL] += ( __mftb() - tmp_time1 );
#endif
				return; 
			}
#endif
			...
			{									
				...
				h->i_skip = 0;							
				x264_dct_t *dct = h->dct_table[h->entropy_idx];		
				x264_macroblock_call_write_cavlc_tpc( h, &h->out.bs, mb, dct );	
				recycle(h,h->entropy_idx);											
				h->entropy_idx++;							
			} 
		}										
	}else{
...
#else
ENTROPY_PPE_PARALLEL();
#endif
}

#ifdef ENTROPY_THREAD
void entropy_spe2(x264_t *h){

start:

       LOCK(&h->entropy);
	while (  h->entropy_idx<h->sh.i_last_mb && h->entropy_idx>=0 ){		
		if( ( h->mb_state_map[(h->entropy_idx)])==DONE_STATE )
		if ( !(tpc_querry( h->tpc_id_table[ h->entropy_idx -1]))  ){		
			ENTROPY_PPE();
...
static void x264_slice_write( x264_t *h )
{
    ...
    int k,l;
      ...
      PREFETCH(	&h->mb_state_map[0] )

    /* init stats */

    uint64_t start_encode = __mftb();
...
#ifdef STAT
    tmp_time1 = tmp_time2 = __mftb();
#endif
    tpc_wait_all();	
#ifdef STAT
     stats[h->sh.i_type][SYNC] += ( __mftb() - tmp_time1 );
#endif
...
#define FILTER(){						\
	if (   	k==(max_row-1)  && 				\
		...
		{	\
	    		const int b_hpel = h->fdec->b_kept_as_ref; \
			const int b_end = l == h->sps->i_mb_height;\
			const int b_deblock = (!h->sh.i_disable_deblocking_filter_idc ) &   (b_hpel || h->param.psz_dump_yuv); ;\
			const int min_y = l - (1 << h->sh.b_mbaff);\
			if( b_deblock )\
			if( !(l & h->sh.b_mbaff) && min_y>0 ){\
			{\
				const int max_y = b_end ? h->sps->i_mb_height : l;\
				int y;\
				for( y = min_y; y < max_y; y += (1 << h->sh.b_mbaff) )\
					entropy_spe(h);\
					x264_frame_deblock_row_tpc( h, y, mb );\
...
#if 0
if (!h->mb_filter_map){
	h->mb_filter_map = h->thread[0]->mb_filter_map;

#ifdef MEMORY_MANAGE
    /* Dynamic memory managment */
    h->i_pool_tail = h->thread[0]->i_pool_tail;
    h->i_pool_head = h->thread[0]->i_pool_head ;

    /* Each entry is a mb+dct+data struct */
    memcpy( &h->mem_pool,  &h->thread[0]->mem_pool, sizeof(h->mem_pool)) ;

    /* stats for dynamic memory usage */
    h->i_pool_count = h->thread[0]->i_pool_count ;
#endif

    /* Bitmaps for tasks */
    h->mb_filter_map = h->thread[0]->mb_filter_map ;

    memcpy( &h->mb_table[X264_MAX_MBS], &h->thread[0]->mb_table[X264_MAX_MBS], sizeof(h->mb_table[X264_MAX_MBS])  );
    memcpy( &h->x264_data_table[X264_MAX_MBS], &h->thread[0]->x264_data_table[X264_MAX_MBS], sizeof( h->x264_data_table[X264_MAX_MBS])  );
    memcpy( &h->x264_filter_data_table_luma[X264_MAX_MBS], &h->thread[0]->x264_filter_data_table_luma[X264_MAX_MBS],  sizeof( h->x264_filter_data_table_luma[X264_MAX_MBS])  );
    memcpy( &h->x264_filter_data_table_chroma[X264_MAX_MBS], &h->thread[0]->x264_filter_data_table_chroma[X264_MAX_MBS],  sizeof( h->x264_filter_data_table_chroma[X264_MAX_MBS])  );
    memcpy( &h->dct_table[X264_MAX_MBS], &h->thread[0]->dct_table[X264_MAX_MBS], sizeof( h->dct_table[X264_MAX_MBS]) );
...
LOCK( &h->mb_encode_mutex);
    ...
    x264_mb_t *mb=&h->mb;
    x264_dct_t *dct ;
    const unsigned int max_col =  h->sh.i_last_mb/h->sps->i_mb_width; 
    const unsigned int max_row =  h->sps->i_mb_width;
    const unsigned int max_iter = max_row+max_col*2;
     ...
     if (!h->entropy_thread){
       LOCK(&h->entropy);
       x264_pthread_create( &h->thread_handle, NULL, (void*) entropy_spe2, h );
       h->entropy_thread=1;
    }
       UNLOCK(&h->entropy);
#endif
    ...
    while( h->mb_state_map[max_col*max_row-1] != DONE_STATE  ){
		/* First loop for analyse  */
                for ( k=i_mb_y,l=0; k>=0 && l<max_col; k-=2,l++){

				entropy_spe(h);
                                if ( k<max_row ){
					/* Caclulate the number of mb */
                                        mb_xy = k + l * (max_row);

					if ( h->mb_state_map[mb_xy]!=UNFETCHED_STATE ) 	 continue;

					entropy_spe(h);
					h->mb_state_map[mb_xy]=ISSUED_STATE;
...
#endif
				        mb = h->mb_table[mb_xy];
					dct = h->dct_table[mb_xy];

		
					/* Load Macroblock Buffers */
        				x264_macroblock_cache_load_tpc( h,  mb );
					entropy_spe(h);
     					x264_macroblock_call_analyse_tpc( h, mb, mb_xy, dct, k, l );
#if 1
					{
						int ik,il, ii_mb_y;
						int mb_xy;
						x264_mb_t *mb; 
						x264_dct_t *dct;
	    					for ( ii_mb_y=i_mb_y-2; (ii_mb_y< i_mb_y+3) && ( ii_mb_y<=max_row+max_col*2)  ; ii_mb_y++)
		               			 for ( ik=ii_mb_y,il=0; ik>=k  && il<max_col; ik-=2,il++){
                		                	if ( ik<max_row ){
								entropy_spe(h);
                                        			mb_xy = ik + il * (max_row);
								if (  h->mb_state_map[mb_xy] == ISSUED_STATE  )
								if ( !(tpc_querry(h->tpc_id_table[ mb_xy ]))){
									mb = h->mb_table[mb_xy];


									x264_macroblock_cache_save_tpc( h, mb ); /* save cache */

									 h->mb_state_map[mb_xy]=DONE_STATE;


									// This is called when top right MB is done
                                        				//mb_xy = ik +1 + il * (max_row);
                                        				mb_xy += 1;
																		
#if 1
									entropy_spe(h);
									if ( ( h->mb_state_map[mb_xy] == UNFETCHED_STATE   )) 
									{
										entropy_spe(h);
                                        					int mb_xy = ik +2 + (il-1) * (max_row);
										if ( mb_xy>0 && ( (ik+2)< max_row )   ){
											if( ( h->mb_state_map[mb_xy]== DONE_STATE )   ){
												entropy_spe(h);
												mb_xy = ik + 1  + (il) * (max_row);
#ifdef MEMORY_MANAGE
												/* Get next data */
												...
												x264_macroblock_cache_load_tpc( h, mb );
												entropy_spe(h);
												x264_macroblock_call_analyse_tpc( h, mb, mb_xy, dct, ik+1, il );
												h->mb_state_map[mb_xy]=ISSUED_STATE;

											}
										}
									}
                                        				mb_xy = ik -2 + (il+1) * (max_row);
									entropy_spe(h);
								   	...
								   	if ( ( h->mb_state_map[mb_xy] == UNFETCHED_STATE )) 
									{
										...
										if ( mb_xy>0 && ( (ik-3)>0  )   ){
											if( (( h->mb_state_map[mb_xy] == DONE_STATE ))   ){
												...
												entropy_spe(h);
												x264_macroblock_cache_load_tpc( h,  mb );
												x264_macroblock_call_analyse_tpc( h, mb, mb_xy, dct, ik-2, il+1 );
				...
				if (  (k>=(max_col-1)) || (l==0) )	x264_fdec_copy_intra( h, l, mb );
...
#ifdef ENTROPY
      tpc_wait(h->tpc_id_table[ h->entropy_idx -1]);
...
#ifndef ENTROPY_THREAD

     for (;h->entropy_idx<h->sh.i_last_mb;){
	      ENTROPY_PPE();
      }



x264_macroblock_call_memset(h, h->mb_state_map,  ALIGN((h->mb.i_mb_count)*sizeof(uint8_t),16)  );
#endif
//exit: ;
...
#ifdef HAVE_PTHREAD2
UNLOCK( &h->mb_encode_mutex);
#endif


      uint64_t end_encode = __mftb();
      total_ticks += end_encode-start_encode;


#ifdef STAT
stats[h->sh.i_type][ENTROPYS] += ( __mftb() - tmp_time1 );
tmp_time1 = __mftb();
#endif
...
////////////////////////////////////////////////////////////////// FOR CLEAN UP
	...
	int curr_task[max_col] ;
	curr_task[0]=0;
	memset(curr_task,0x00, sizeof(int)*max_col );
	i_mb_y = 0;
	i_mb_x = 0;


	while( h->mb_state_map[max_col*max_row-1] != DONE_STATE  ){
		for ( k=active_low; k<=active_high; k++){
			mb_xy = curr_task[k];
			i_mb_y = mb_xy/max_row;
			i_mb_x = mb_xy%max_row;
			...
			if ( h->mb_state_map[mb_xy]==ISSUED_STATE ){
				if ( !(tpc_querry(h->tpc_id_table[ mb_xy ]))){
					...
					h->mb_state_map[mb_xy] = DONE_STATE;
					curr_task[k]++;
					if ( (mb->i_mb_x==1) && (mb->i_mb_y <(max_col-1)) ){
						 active_high++;
						curr_task[k+1] = ( mb->i_mb_y + 1 )*max_row; 
					}
					if ( mb->i_mb_x==(max_row-1) ) active_low ++;
			...
			if ( h->mb_state_map[mb_xy]==UNFETCHED_STATE ){
				...
				if (i_mb_y>0){
					int top_left = i_mb_x + 1  + max_row*(i_mb_y-1);
					if ( h->mb_state_map[top_left]==DONE_STATE ) {
						/* Get next data */
						alloc_mb(h,mb_xy); 
						...
						/* Load Macroblock Buffers */
						x264_macroblock_cache_load_tpc( h,  mb );
						x264_macroblock_call_analyse_tpc( h, mb, mb_xy, dct, mb->i_mb_x, mb->i_mb_y );
						h->mb_state_map[mb_xy]=ISSUED_STATE;
					}
				}else   if(i_mb_y==0 || i_mb_x==(max_row-1)) {
					/* Get next data */
					alloc_mb(h,mb_xy); 
			...
			if (  (i_mb_x>=(max_col-1)) || (i_mb_y=0) )	x264_fdec_copy_intra( h, i_mb_y, mb );
			entropy_spe(h);
			FILTER();
		}	
		tpc_wait(h->tpc_id_table[ curr_task[active_low] ]);
	}
	...
	for (i=h->entropy_idx;i<h->sh.i_last_mb;i++){
		ENTROPY_PPE();
	}
                ...
                for ( k=i_mb_y,l=0; k>=0 ; k-=2,l++){
					...
					/* Load Macroblock Buffers */
        				x264_macroblock_cache_load_tpc( h,  mb );
     					...
     					x264_macroblock_call_analyse_tpc( h, mb, mb_xy, dct, k, l );
#else
					x264_macroblock_analyse_tpc( h, mb, mb_xy, dct );
	        			x264_macroblock_encode_tpc( h, mb, dct);
#endif
...
/////////////////////////////////////////////////////////////////
...
#ifdef STAT
                stats[h->sh.i_type][I_FRAME] += ( __mftb() - tmp_time1 );
                tmp_time1 = __mftb();
#endif
    ...
    {
        if( h->i_skip > 0 )
            bs_write_ue( &h->out.bs, h->i_skip );  /* last skip run */
    ...
    for (i=0;i<h->mb.i_mb_count;i++){
          if (h->mb_table[i]){
		h->mem_pool[h->i_pool_head] = h->mb_table[i];
    ...
    x264_macroblock_call_memset(h, h->mb_table,    (h->mb.i_mb_count) *sizeof( x264_mb_t*)  );
 //   x264_macroblock_call_memset(h, h->dct_table,  (h->mb.i_mb_count) *sizeof( x264_dct_t*)  );
//    x264_macroblock_call_memset(h, h->x264_data_table, (h->mb.i_mb_count) *sizeof( x264_data_t*)  );

#endif


#ifdef STAT
     stats[h->sh.i_type][MANAGE] += ( __mftb() - tmp_time2 );
#endif
                             ...
                             x264_picture_t *pic_out )
{
    ...
    //x264_macroblock_call_memset(h, &h->stat.frame,0, sizeof(h->stat.frame));
...
#ifdef TPC
    x264_reference_build_list_tpc( h );
#endif
...
#ifdef TPC
   //     x264_macroblock_bipred_init_tpc( h );
#endif
...
//        x264_macroblock_cache_end( h->thread[i] );
   ...
   printf("Actual slice encode ticks: %llu \n", total_ticks);
