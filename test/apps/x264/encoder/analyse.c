...
#ifdef TPC
static void x264_analyse_update_cache_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb );
#endif
...
#else
        h->mb.mv_min[0] =  X264_MAX( 4*( -16*(h->mb.i_mb_x) - 24 ) ,    -h->param.analyse.i_me_range  );
        h->mb.mv_max[0] = X264_MIN( 4*( 16*(  h->sps->i_mb_width - h->mb.i_mb_x - 1 ) + 24 ) ,   h->param.analyse.i_me_range +32 );
#endif
...
#if 0
            h->mb.mv_min_spel[1] = X264_MAX(  CLIP_FMV( h->mb.mv_min[1] ),  -h->param.analyse.i_me_range       );
            h->mb.mv_max_spel[1] = X264_MIN(  CLIP_FMV( h->mb.mv_max[1] ),   h->param.analyse.i_me_range +32    );
#endif
...
static void x264_mb_analyse_intra_chroma_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb )
{
    int i;
...
static void x264_mb_analyse_intra_tpc( x264_t *h, x264_mb_analysis_t *a, int i_satd_inter, x264_mb_t *mb )
{
...
/*
 * NOTE: Disabled other than 16x16 intra analysis
 *
 * */
...
static void x264_mb_analyse_inter_direct_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb )
{
    /* Assumes that fdec still contains the results of
     * x264_mb_predict_mv_direct16x16 and x264_mb_mc */

    uint8_t **p_fenc = mb->pic.p_fenc;
    uint8_t **p_fdec = mb->pic.p_fdec;
    int i;
...
/* Pre-calculate fenc satd scores for psy RD, minus DC coefficients */
static inline void x264_mb_cache_fenc_satd_tpc( x264_t *h, x264_mb_t *mb )
{
    DECLARE_ALIGNED_16(uint8_t zero[16]) = {0};
    uint8_t *fenc;
    int x, y, satd_sum = 0, sa8d_sum = 0;
    if( h->param.analyse.i_trellis == 2 && mb->i_psy_trellis )
        x264_psy_trellis_init( h, h->param.analyse.b_transform_8x8 );
    if( !mb->i_psy_rd )
        return;
    for( y = 0; y < 4; y++ )
        for( x = 0; x < 4; x++ )
        {
            ...
            mb->pic.fenc_satd[y][x] = h->pixf.satd[PIXEL_4x4]( zero, 0, fenc, FENC_STRIDE )
                                      - (h->pixf.sad[PIXEL_4x4]( zero, 0, fenc, FENC_STRIDE )>>1);
    ...
    for( y = 0; y < 2; y++ )
        for( x = 0; x < 2; x++ )
        {
            ...
            mb->pic.fenc_sa8d[y][x] = h->pixf.sa8d[PIXEL_8x8]( zero, 0, fenc, FENC_STRIDE )
                                      - (h->pixf.sad[PIXEL_8x8]( zero, 0, fenc, FENC_STRIDE )>>2);
...
static void x264_mb_analyse_init_tpc( x264_t *h, x264_mb_analysis_t *a,  x264_mb_t *mb )
{
    int i_qp = mb->i_qp;

    /* conduct the analysis using this lamda and QP */
    a->i_qp =  i_qp;
    mb->i_chroma_qp = h->chroma_qp_table[i_qp];
    a->i_lambda = x264_lambda_tab[i_qp];
    a->i_lambda2 = x264_lambda2_tab[i_qp];
    a->b_mbrd = h->param.analyse.i_subpel_refine >= 6 &&
                ( h->sh.i_type != SLICE_TYPE_B || h->param.analyse.b_bframe_rdo );

    mb->i_me_method = h->param.analyse.i_me_method;
    mb->i_subpel_refine = h->param.analyse.i_subpel_refine;
    mb->b_chroma_me = h->param.analyse.b_chroma_me && h->sh.i_type == SLICE_TYPE_P
...
#define CLIP_FMV(mv) x264_clip3( mv, -i_fmv_range, i_fmv_range-1 )
/*
 * If we need to compare the two versions we must set different region. The 
 * loca store size limits the tranfered region.
 */
#if 1
        mb->mv_min[0] = 4*( -16*(mb->i_mb_x) - 24 );
        mb->mv_max[0] = 4*( 16*( h->sps->i_mb_width - mb->i_mb_x - 1 ) + 24 );
...
#else
        mb->mv_min[0] = 4*( -16*(h->mb.i_mb_x) - 24 );
        mb->mv_max[0] = 4*( 16*( h->sps->i_mb_width - h->mb.i_mb_x - 1 ) + 24 );
        mb->mv_min_spel[0] = CLIP_FMV( h->mb.mv_min[0] );
        mb->mv_max_spel[0] = CLIP_FMV( h->mb.mv_max[0] );
        mb->mv_min_fpel[0] = (h->mb.mv_min_spel[0]>>2) + i_fpel_border;
        mb->mv_max_fpel[0] = (h->mb.mv_max_spel[0]>>2) - i_fpel_border;
#endif

        if( mb->i_mb_x == 0)
        {
            int mb_y = mb->i_mb_y >> h->sh.b_mbaff;
            int mb_height = h->sps->i_mb_height >> h->sh.b_mbaff;
            int thread_mvy_range = i_fmv_range;

            if( h->param.i_threads > 1 )
            {
                int pix_y = (mb->i_mb_y | mb->b_interlaced) * 16;
                int thresh = pix_y + h->param.analyse.i_mv_range_thread;
                for( i = (h->sh.i_type == SLICE_TYPE_B); i >= 0; i-- )
                {
                    x264_frame_t **fref = i ? h->fref1 : h->fref0;
            ...
            mb->mv_min_spel[1] = x264_clip3( mb->mv_min[1], X264_MAX(4*(-512+i_spel_border), -i_fmv_range), i_fmv_range );
            mb->mv_max_spel[1] = CLIP_FMV( mb->mv_max[1] );
            mb->mv_max_spel[1] = X264_MIN( mb->mv_max_spel[1], thread_mvy_range*4 );
...
#undef CLIP_FMV
        ...
        /* Fast intra decision */
        if( mb->i_mb_xy - h->sh.i_first_mb > 4 )
        {
            if(   IS_INTRA( mb->i_mb_type_left )
               || IS_INTRA( mb->i_mb_type_top )
               || IS_INTRA( mb->i_mb_type_topleft )
               || IS_INTRA( mb->i_mb_type_topright )
               || (h->sh.i_type == SLICE_TYPE_P && IS_INTRA( h->fref0[0]->mb_type[mb->i_mb_xy] ))
               || (mb->i_mb_xy - h->sh.i_first_mb < 3*(h->stat.frame.i_mb_count[I_4x4] + h->stat.frame.i_mb_count[I_8x8] + h->stat.frame.i_mb_count[I_16x16])) )
            { /* intra is likely */ }
            else
            {
                a->b_fast_intra = 1;
...
static void x264_mb_analyse_inter_p16x16_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb )
{
    x264_me_t m;
    int i_ref, i_mvc;
    DECLARE_ALIGNED_4( int16_t mvc[8][2] );
    ...
    for( i_ref = 0; i_ref < mb->pic.i_fref[0]; i_ref++ )
    {
        ...
        x264_mb_predict_mv_ref16x16_tpc( h, 0, i_ref, mvc, &i_mvc, mb);
        x264_me_search_ref_tpc( h, &m, mvc, i_mvc, p_halfpel_thresh, mb );
        ...
        if( i_ref == 0
            ...
            && x264_macroblock_probe_pskip_tpc( h, mb ) )
        {
            mb->i_type = P_SKIP;
            x264_analyse_update_cache( h, a );
            assert( mb->cache.pskip_mv[1] <= mb->mv_max_spel[1] || h->param.i_threads == 1 );
            return;
        }
    ...
    assert( a->l0.me16x16.mv[1] <= mb->mv_max_spel[1] || h->param.i_threads == 1 );
...
static void x264_mb_analyse_inter_b16x16_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb )
{
    DECLARE_ALIGNED_16( uint8_t pix0[16*16] );
    DECLARE_ALIGNED_16( uint8_t pix1[16*16] );
    uint8_t *src0, *src1;
    ...
    for( i_ref = 0; i_ref < mb->pic.i_fref[1]; i_ref++ )
    {
        ...
        x264_mb_predict_mv_ref16x16_tpc( h, 1, i_ref, mvc, &i_mvc, mb );
        x264_me_search_ref_tpc( h, &m, mvc, i_mvc, p_halfpel_thresh, mb );
    ...
    h->mc.avg[PIXEL_16x16]( pix0, 16, src0, stride0, src1, stride1, mb->bipred_weight[a->l0.i_ref][a->l1.i_ref] );

    a->i_cost16x16bi = h->pixf.mbcmp[PIXEL_16x16]( mb->pic.p_fenc[0], FENC_STRIDE, pix0, 16 )
                     + REF_COST( 0, a->l0.i_ref )
                     + REF_COST( 1, a->l1.i_ref )
                     + a->l0.me16x16.cost_mv
                     + a->l1.me16x16.cost_mv;
...
static void x264_mb_analyse_b_rd_tpc( x264_t *h, x264_mb_analysis_t *a, int i_satd_inter, x264_mb_t *mb, x264_dct_t *dct )
{
    int thresh = i_satd_inter * (17 + (!!mb->i_psy_rd))/16;
    ...
    if( a->b_direct_available && a->i_rd16x16direct == COST_MAX )
    {
        ...
        /* Requires b-rdo to be done before intra analysis */
        mb->b_skip_mc = 1;
        x264_analyse_update_cache_tpc( h, a, mb );
        a->i_rd16x16direct = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
        mb->b_skip_mc = 0;
    }
    ...
    if( a->l0.me16x16.cost <= thresh && a->l0.i_rd16x16 == COST_MAX )
    {
        mb->i_type = B_L0_L0;
        x264_analyse_update_cache_tpc( h, a, mb );
        a->l0.i_rd16x16 = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
    }
    ...
    if( a->l1.me16x16.cost <= thresh && a->l1.i_rd16x16 == COST_MAX )
    {
        mb->i_type = B_L1_L1;
        x264_analyse_update_cache_tpc( h, a, mb );
        a->l1.i_rd16x16 = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
    }
    ...
    if( a->i_cost16x16bi <= thresh && a->i_rd16x16bi == COST_MAX )
    {
        mb->i_type = B_BI_BI;
        x264_analyse_update_cache_tpc( h, a, mb );
        a->i_rd16x16bi = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
    }
    ...
    if( a->i_cost8x8bi <= thresh && a->i_rd8x8bi == COST_MAX )
    {
        ...
        x264_analyse_update_cache_tpc( h, a, mb );
        a->i_rd8x8bi = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
        x264_macroblock_cache_skip( h, 0, 0, 4, 4, 0 );
    }
    ...
    if( a->i_cost16x8bi <= thresh && a->i_rd16x8bi == COST_MAX )
    {
        mb->i_type = a->i_mb_type16x8;
        mb->i_partition = D_16x8;
        x264_analyse_update_cache_tpc( h, a, mb );
        a->i_rd16x8bi = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
    }

    /* 8x16 */
    if( a->i_cost8x16bi <= thresh && a->i_rd8x16bi == COST_MAX )
    {
        mb->i_type = a->i_mb_type8x16;
        mb->i_partition = D_8x16;
        x264_analyse_update_cache_tpc( h, a, mb );
        a->i_rd8x16bi = x264_rd_cost_mb_tpc( h, a->i_lambda2, mb, dct );
    }
}



static void x264_intra_rd_refine_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb, x264_dct_t *dct )
{
    uint8_t  *p_src = mb->pic.p_fenc[0];
    uint8_t  *p_dst = mb->pic.p_fdec[0];
    ...
    if( mb->i_type == I_16x16 )
    {
        ...
        for( i = 0; i < i_max; i++ )
        {
            ...
            if( i_mode == old_pred_mode || a->i_satd_i16x16_dir[i_mode] > i_thresh )
                continue;
            mb->i_intra16x16_pred_mode = i_mode;
            i_satd = x264_rd_cost_mb( h, a->i_lambda2 );
            COPY2_IF_LT( i_best, i_satd, a->i_predict16x16, i_mode );
        }
    }
    else if( mb->i_type == I_4x4 )
    {
        ...
        for( idx = 0; idx < 16; idx++ )
        {
            ...
            predict_4x4_mode_available( mb->i_neighbour4[idx], predict_mode, &i_max );

            if( (mb->i_neighbour4[idx] & (MB_TOPRIGHT|MB_TOP)) == MB_TOP )
                /* emulate missing topright samples */
                *(uint32_t*) &p_dst_by[4 - FDEC_STRIDE] = p_dst_by[3 - FDEC_STRIDE] * 0x01010101U;
            ...
            for( i = 0; i < i_max; i++ )
            {
                ...
                if( i_best > i_satd )
                {
                    ...
                    pels[2] = *(uint32_t*)(p_dst_by+2*FDEC_STRIDE);
                    pels[3] = *(uint32_t*)(p_dst_by+3*FDEC_STRIDE);
                    i_nnz = mb->cache.non_zero_count[x264_scan8[idx]];
                }
            }
            ...
            *(uint32_t*)(p_dst_by+2*FDEC_STRIDE) = pels[2];
            *(uint32_t*)(p_dst_by+3*FDEC_STRIDE) = pels[3];
            mb->cache.non_zero_count[x264_scan8[idx]] = i_nnz;

            mb->cache.intra4x4_pred_mode[x264_scan8[idx]] = a->i_predict4x4[idx];
        }
    }
    else if( mb->i_type == I_8x8 )
    {
        assert( mb->i_type != I_8x8  );
        DECLARE_ALIGNED_16( uint8_t edge[33] );
        for( idx = 0; idx < 4; idx++ )
        {
            uint64_t pels_h = 0;
            ...
            p_src_by = p_src + 8*x + 8*y*FENC_STRIDE;
            p_dst_by = p_dst + 8*x + 8*y*FDEC_STRIDE;
            predict_4x4_mode_available( mb->i_neighbour8[idx], predict_mode, &i_max );
            x264_predict_8x8_filter( p_dst_by, edge, mb->i_neighbour8[idx], ALL_NEIGHBORS );
                ...
                if( i_best > i_satd )
                {
                    ...
                    if( !(idx&1) )
                        for( j=0; j<7; j++ )
                            pels_v[j] = p_dst_by[7+j*FDEC_STRIDE];
                    for( j=0; j<3; j++ )
                        i_nnz[j] = mb->cache.non_zero_count[x264_scan8[4*idx+j+1]];
                }
            }
            ...
            *(uint64_t*)(p_dst_by+7*FDEC_STRIDE) = pels_h;
            if( !(idx&1) )
                for( j=0; j<7; j++ )
                    p_dst_by[7+j*FDEC_STRIDE] = pels_v[j];
            for( j=0; j<3; j++ )
                mb->cache.non_zero_count[x264_scan8[4*idx+j+1]] = i_nnz[j];
    ...
    if( i_max > 1 )
    {
        ...
        if( i_max > 0 )
        {
            int i_chroma_lambda = x264_lambda2_tab[mb->i_chroma_qp];
            /* the previous thing encoded was x264_intra_rd(), so the pixels and
             * coefs for the current chroma mode are still around, so we only
             * have to recount the bits. */
            i_best = x264_rd_cost_i8x8_chroma_tpc( h, i_chroma_lambda, a->i_predict8x8chroma, 0, mb, dct );
            for( i = 0; i < i_max; i++ )
            {
                i_mode = predict_mode[i];
                 ...
                 * probably any mode with a residual will be worse.
                 * so avoid dct on the remaining modes to improve speed. */
                i_satd = x264_rd_cost_i8x8_chroma_tpc( h, i_chroma_lambda, i_mode, mb->i_cbp_chroma != 0x00, mb, dct );
                COPY2_IF_LT( i_best, i_satd, a->i_predict8x8chroma, i_mode );
            }
            mb->i_chroma_pred_mode = a->i_predict8x8chroma;
        }
 ...
 * x264_macroblock_analyse: 
 *****************************************************************************/
void x264_macroblock_analyse_tpc( x264_t *h, x264_mb_t *mb, int mb_i_xy, x264_dct_t *dct )
{
    int i_cost = COST_MAX;
    int i;
    ...
    mb->i_qp = x264_ratecontrol_qp( h );
    if( h->param.rc.i_aq_mode )
        x264_adaptive_quant( h );


    x264_mb_analyse_init_tpc( h, analysis, mb );
    ...
    if( h->sh.i_type == SLICE_TYPE_I )
    {
        ...
        i_cost = analysis->i_satd_i16x16;
        mb->i_type = I_16x16;
        COPY2_IF_LT( i_cost, analysis->i_satd_i4x4, mb->i_type, I_4x4 );
        COPY2_IF_LT( i_cost, analysis->i_satd_i8x8, mb->i_type, I_8x8 );
        if( analysis->i_satd_pcm < i_cost )
            mb->i_type = I_PCM;

        else if( mb->i_subpel_refine >= 7 )
            x264_intra_rd_refine( h, analysis );
    }
    else if( h->sh.i_type == SLICE_TYPE_P )
    {
        ...
        h->mc.prefetch_ref( mb->pic.p_fref[0][0][mb->i_mb_x&3], mb->pic.i_stride[0], 0 );
        ...
        if( h->param.analyse.b_fast_pskip )
        {
            if( h->param.i_threads > 1 && mb->cache.pskip_mv[1] > mb->mv_max_spel[1] )
                // FIXME don't need to check this if the reference frame is done
                {}
            else if( h->param.analyse.i_subpel_refine >= 3 )
                analysis->b_try_pskip = 1;
                ...
                b_skip = x264_macroblock_probe_pskip_tpc( h, mb );
        }

        h->mc.prefetch_ref( mb->pic.p_fref[0][0][mb->i_mb_x&3], mb->pic.i_stride[0], 1 );
        ...
        else
        {
            ...
            x264_mb_analyse_inter_p16x16_tpc( h, analysis, mb );

            if( mb->i_type == P_SKIP )
                return;

/* NOTE: SUB 16x16 DISABLED */
#if 0
            if( flags & X264_ANALYSE_PSUB16x16 )
            {
                if( h->param.analyse.b_mixed_references )
...
/* NOTE: SUB 16x16 DISABLED */
            ...
            {
                x264_me_refine_qpel_tpc( h, &analysis->l0.me16x16, mb);
                i_cost = analysis->l0.me16x16.cost;
            }
            else if( i_partition == D_16x8 )
            {
                x264_me_refine_qpel_tpc( h, &analysis->l0.me16x8[0],mb );
                x264_me_refine_qpel_tpc( h, &analysis->l0.me16x8[1],mb );
                i_cost = analysis->l0.me16x8[0].cost + analysis->l0.me16x8[1].cost;
            }
            else if( i_partition == D_8x16 )
            {
                x264_me_refine_qpel_tpc( h, &analysis->l0.me8x16[0], mb);
                x264_me_refine_qpel_tpc( h, &analysis->l0.me8x16[1], mb );
                i_cost = analysis->l0.me8x16[0].cost + analysis->l0.me8x16[1].cost;
            }
            else if( i_partition == D_8x8 )
            {
                ...
                for( i8x8 = 0; i8x8 < 4; i8x8++ )
                {
                    switch( mb->i_sub_partition[i8x8] )
                    {
                        case D_L0_8x8:
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me8x8[i8x8], mb );
                            i_cost += analysis->l0.me8x8[i8x8].cost;
                            break;
                        case D_L0_8x4:
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me8x4[i8x8][0], mb);
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me8x4[i8x8][1], mb);
                            i_cost += analysis->l0.me8x4[i8x8][0].cost +
                                      analysis->l0.me8x4[i8x8][1].cost;
                            break;
                        case D_L0_4x8:
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me4x8[i8x8][0],mb );
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me4x8[i8x8][1], mb);
                            i_cost += analysis->l0.me4x8[i8x8][0].cost +
                                      analysis->l0.me4x8[i8x8][1].cost;
                            break;

                        case D_L0_4x4:
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me4x4[i8x8][0],mb );
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me4x4[i8x8][1],mb );
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me4x4[i8x8][2],mb );
                            x264_me_refine_qpel_tpc( h, &analysis->l0.me4x4[i8x8][3],mb );
                            i_cost += analysis->l0.me4x4[i8x8][0].cost +
                                      analysis->l0.me4x4[i8x8][1].cost +
                                      analysis->l0.me4x4[i8x8][2].cost +
                                      analysis->l0.me4x4[i8x8][3].cost;
            ...
            if( mb->b_chroma_me )
            {
                ...
                //x264_mb_analyse_intra( h, analysis, i_cost );
                x264_mb_analyse_intra_tpc( h, analysis, i_cost, mb );
            ...
            if( mb->i_subpel_refine >= 7 && mb->i_type != I_PCM )
            {
                ...
                {
                    mb->i_sub_partition[0] = mb->i_sub_partition[1] =
                    mb->i_sub_partition[2] = mb->i_sub_partition[3] = D_L0_8x8;
                    x264_macroblock_cache_ref( h, 0, 0, 4, 2, 0, analysis->l0.me16x8[0].i_ref );
                    x264_macroblock_cache_ref( h, 0, 2, 4, 2, 0, analysis->l0.me16x8[1].i_ref );
                    x264_me_refine_qpel_rd( h, &analysis->l0.me16x8[0], analysis->i_lambda2, 0 );
                    x264_me_refine_qpel_rd( h, &analysis->l0.me16x8[1], analysis->i_lambda2, 2 );
                ...
                else if( i_partition == D_8x8 )
                {
                    ...
                    x264_analyse_update_cache( h, analysis );
                    for( i8x8 = 0; i8x8 < 4; i8x8++ )
                         if( mb->i_sub_partition[i8x8] == D_L0_8x8 )
                             x264_me_refine_qpel_rd( h, &analysis->l0.me8x8[i8x8], analysis->i_lambda2, i8x8 );
                }
            }
        }
    ...
    else if( h->sh.i_type == SLICE_TYPE_B )
    {
        ...
        // TODO: Add later support for this
        if( analysis->b_mbrd )
            x264_mb_cache_fenc_satd_tpc( h, mb );
        ...
        if( mb->b_direct_auto_write )
        {
            ...
            for( i = 0; i < 2; i++ )
            {
                int b_changed = 1;
                h->sh.b_direct_spatial_mv_pred ^= 1;
                analysis->b_direct_available = x264_mb_predict_mv_direct16x16_tpc( h, i && analysis->b_direct_available ? &b_changed : NULL, mb );
                if( analysis->b_direct_available )
                {
                    if( b_changed )
                    {
                        x264_mb_mc( h );
                        b_skip = x264_macroblock_probe_bskip_tpc( h, mb );
                    }
                    h->stat.frame.i_direct_score[ h->sh.b_direct_spatial_mv_pred ] += b_skip;
                }
                else
            ...
            analysis->b_direct_available = x264_mb_predict_mv_direct16x16_tpc( h, NULL, mb );
	...
	/* Is direct available ? */        
 	...
 	/* Here we enter in main analyse */
        if( !b_skip )
        {
            ...
            /* direct must be first */
            if( analysis->b_direct_available )
                x264_mb_analyse_inter_direct_tpc( h, analysis, mb );

            x264_mb_analyse_inter_b16x16_tpc( h, analysis, mb );
            ...
            if( analysis->b_mbrd && analysis->i_cost16x16direct <= i_cost * 33/32 )
            {
                x264_mb_analyse_b_rd_tpc( h, analysis, i_cost, mb, dct );
                if( i_bskip_cost < analysis->i_rd16x16direct &&
                    i_bskip_cost < analysis->i_rd16x16bi &&
                    i_bskip_cost < analysis->l0.i_rd16x16 &&
                    i_bskip_cost < analysis->l1.i_rd16x16 )
                {
                    mb->i_type = B_SKIP;
                    x264_analyse_update_cache_tpc( h, analysis, mb );
                    return;
                }
            }

            if( flags & X264_ANALYSE_BSUB16x16 ) /// TODO: future support !
            {
                x264_mb_analyse_inter_b8x8( h, analysis );
                if( analysis->i_cost8x8bi < i_cost )
                {
                    ...
                    if( mb->i_sub_partition[0] == mb->i_sub_partition[1] ||
                        mb->i_sub_partition[2] == mb->i_sub_partition[3] )
                    {
                        x264_mb_analyse_inter_b16x8( h, analysis );
                        COPY3_IF_LT( i_cost, analysis->i_cost16x8bi,
                                     i_type, analysis->i_mb_type16x8,
                    ...
                    if( mb->i_sub_partition[0] == mb->i_sub_partition[2] ||
                        mb->i_sub_partition[1] == mb->i_sub_partition[3] )
                    {
                        x264_mb_analyse_inter_b8x16( h, analysis );
                        COPY3_IF_LT( i_cost, analysis->i_cost8x16bi,
                                     i_type, analysis->i_mb_type8x16,
            ...
            else if( i_partition == D_16x16 )
            {
                ...
                {
                    x264_me_refine_qpel_tpc( h, &analysis->l1.me16x16, mb );
                    i_cost = analysis->l1.me16x16.cost
                           + analysis->i_lambda * i_mb_b_cost_table[B_L1_L1];
                }
                else if( i_type == B_BI_BI )
            ...
            else if( i_partition == D_16x8 )
            {
                ...
                {
                    if( analysis->i_mb_partition16x8[i] != D_L1_8x8 )
                        x264_me_refine_qpel_tpc( h, &analysis->l0.me16x8[i], mb );
                    if( analysis->i_mb_partition16x8[i] != D_L0_8x8 )
                        x264_me_refine_qpel_tpc( h, &analysis->l1.me16x8[i], mb );
                }
            }
            else if( i_partition == D_8x16 )
            {
                ...
                {
                    if( analysis->i_mb_partition8x16[i] != D_L1_8x8 )
                        x264_me_refine_qpel_tpc( h, &analysis->l0.me8x16[i], mb );
                    if( analysis->i_mb_partition8x16[i] != D_L0_8x8 )
                        x264_me_refine_qpel_tpc( h, &analysis->l1.me8x16[i], mb );
                }
            }
            else if( i_partition == D_8x8 )
            {
                for( i=0; i<4; i++ )
                {
                    ...
                    int i_part_cost_old;
                    int i_type_cost;
                    int i_part_type = mb->i_sub_partition[i];
                    int b_bidir = (i_part_type == D_BI_8x8);
            ...
            {
                i_satd_inter = i_cost;
                x264_mb_analyse_b_rd_tpc( h, analysis, i_satd_inter, mb, dct );
                i_type = B_SKIP;
                i_cost = i_bskip_cost;
                i_partition = D_16x16;
                COPY2_IF_LT( i_cost, analysis->l0.i_rd16x16, i_type, B_L0_L0 );
...
/// TODO: ADD SUPPORT FOR INTRA
#if 1
            memcpy(&h->mb,mb,sizeof(*mb));
            x264_mb_analyse_intra( h, analysis, i_satd_inter );
            memcpy(mb,&h->mb,sizeof(*mb));
#endif
		...
		assert(0);
            ...
            mb->i_partition = i_partition;
#if 0
            if( analysis->b_mbrd && mb->i_subpel_refine >= 7 && IS_INTRA( i_type ) && i_type != I_PCM )
                x264_intra_rd_refine( h, analysis );
            else if( h->param.analyse.b_bidir_me )
                refine_bidir( h, analysis );
#endif
    ...
    mb->b_trellis = h->param.analyse.i_trellis;
    mb->b_noise_reduction = !!h->param.analyse.i_noise_reduction;
    if( !IS_SKIP(mb->i_type) && mb->i_psy_trellis && h->param.analyse.i_trellis == 1 )
        x264_psy_trellis_init( h, 0 );
    if( mb->b_trellis == 1 || mb->b_noise_reduction )
        mb->i_skip_intra = 0;
}

static inline void x264_mb_cache_mv_p8x8_tpc(  x264_mb_analysis_t *a, int i, x264_mb_t *mb )
{
    const int x = 2*(i%2);
    const int y = 2*(i/2);
    ...
    {
        case D_L0_8x8:
            x264_macroblock_cache_mv_ptr_tpc( x, y, 2, 2, 0, a->l0.me8x8[i].mv, mb );
            break;
        case D_L0_8x4:
            x264_macroblock_cache_mv_ptr_tpc( x, y+0, 2, 1, 0, a->l0.me8x4[i][0].mv, mb );
            x264_macroblock_cache_mv_ptr_tpc( x, y+1, 2, 1, 0, a->l0.me8x4[i][1].mv, mb );
            break;
        case D_L0_4x8:
            x264_macroblock_cache_mv_ptr_tpc( x+0, y, 1, 2, 0, a->l0.me4x8[i][0].mv, mb );
            x264_macroblock_cache_mv_ptr_tpc( x+1, y, 1, 2, 0, a->l0.me4x8[i][1].mv, mb );
            break;
        case D_L0_4x4:
            x264_macroblock_cache_mv_ptr_tpc( x+0, y+0, 1, 1, 0, a->l0.me4x4[i][0].mv, mb );
            x264_macroblock_cache_mv_ptr_tpc( x+1, y+0, 1, 1, 0, a->l0.me4x4[i][1].mv, mb );
            x264_macroblock_cache_mv_ptr_tpc( x+0, y+1, 1, 1, 0, a->l0.me4x4[i][2].mv, mb );
            x264_macroblock_cache_mv_ptr_tpc( x+1, y+1, 1, 1, 0, a->l0.me4x4[i][3].mv, mb );
            break;
        default:
#if 1
            assert(0);
...
#define CACHE_MV_BI_TPC(x,y,dx,dy,me0,me1,part) \
    if( x264_mb_partition_listX_table[0][part] ) \
    { \
        x264_macroblock_cache_ref_tpc( x,y,dx,dy, 0, a->l0.i_ref, mb ); \
        x264_macroblock_cache_mv_ptr_tpc( x,y,dx,dy, 0, me0.mv, mb ); \
    } \
    else \
    { \
        x264_macroblock_cache_ref_tpc(  x,y,dx,dy, 0, -1, mb ); \
        x264_macroblock_cache_mv_tpc(   x,y,dx,dy, 0,  0, mb ); \
        if( b_mvd ) \
            x264_macroblock_cache_mvd_tpc(  x,y,dx,dy, 0, 0, mb ); \
    } \
    if( x264_mb_partition_listX_table[1][part] ) \
    { \
        x264_macroblock_cache_ref_tpc(  x,y,dx,dy, 1, a->l1.i_ref, mb ); \
        x264_macroblock_cache_mv_ptr_tpc(  x,y,dx,dy, 1, me1.mv, mb ); \
    } \
    else \
    { \
        x264_macroblock_cache_ref_tpc(  x,y,dx,dy, 1, -1, mb ); \
        x264_macroblock_cache_mv_tpc(   x,y,dx,dy, 1,  0, mb ); \
        if( b_mvd ) \
            x264_macroblock_cache_mvd_tpc( x,y,dx,dy, 1, 0, mb ); \
    }

static inline void x264_mb_cache_mv_b8x8_tpc( x264_t *h, x264_mb_analysis_t *a, int i, int b_mvd, x264_mb_t *mb )
{
    int x = (i%2)*2;
    int y = (i/2)*2;
    if( mb->i_sub_partition[i] == D_DIRECT_8x8 )
    {
        x264_mb_load_mv_direct8x8_tpc(i, mb );
        if( b_mvd )
        {
            x264_macroblock_cache_mvd_tpc(  x, y, 2, 2, 0, 0, mb );
            x264_macroblock_cache_mvd_tpc(  x, y, 2, 2, 1, 0, mb );
            x264_macroblock_cache_skip_tpc( x, y, 2, 2, 1, mb );
        }
    }
    else
    {
        CACHE_MV_BI_TPC( x, y, 2, 2, a->l0.me8x8[i], a->l1.me8x8[i], mb->i_sub_partition[i] );
    }
}
static inline void x264_mb_cache_mv_b16x8_tpc(  x264_mb_analysis_t *a, int i, int b_mvd, x264_mb_t *mb )
{
    CACHE_MV_BI_TPC( 0, 2*i, 4, 2, a->l0.me16x8[i], a->l1.me16x8[i], a->i_mb_partition16x8[i] );
}
static inline void x264_mb_cache_mv_b8x16_tpc(  x264_mb_analysis_t *a, int i, int b_mvd, x264_mb_t *mb )
{
    CACHE_MV_BI_TPC( 2*i, 0, 2, 4, a->l0.me8x16[i], a->l1.me8x16[i], a->i_mb_partition8x16[i] );
}
#undef CACHE_MV_BI_TPC
...
static void x264_analyse_update_cache_tpc( x264_t *h, x264_mb_analysis_t *a, x264_mb_t *mb )
{
    ...
    switch( mb->i_type )
    {
        case I_4x4:
            for( i = 0; i < 16; i++ )
                mb->cache.intra4x4_pred_mode[x264_scan8[i]] = a->i_predict4x4[i];

            x264_mb_analyse_intra_chroma_tpc( h, a, mb ); 
            break;
        case I_8x8:
            for( i = 0; i < 4; i++ )
                x264_macroblock_cache_intra8x8_pred( h, 2*(i&1), 2*(i>>1), a->i_predict8x8[i] );  /// TODO: CHECK IT!
        ...
        case P_L0:
            switch( mb->i_partition )
            {
                ...
                case D_16x8:
                    x264_macroblock_cache_ref_tpc( 0, 0, 4, 2, 0, a->l0.me16x8[0].i_ref, mb );
                    x264_macroblock_cache_ref_tpc( 0, 2, 4, 2, 0, a->l0.me16x8[1].i_ref, mb );
                    x264_macroblock_cache_mv_ptr_tpc( 0, 0, 4, 2, 0, a->l0.me16x8[0].mv, mb );
                    x264_macroblock_cache_mv_ptr_tpc( 0, 2, 4, 2, 0, a->l0.me16x8[1].mv, mb );
                    break;

                case D_8x16:
                    x264_macroblock_cache_ref_tpc( 0, 0, 2, 4, 0, a->l0.me8x16[0].i_ref, mb );
                    x264_macroblock_cache_ref_tpc( 2, 0, 2, 4, 0, a->l0.me8x16[1].i_ref, mb );
                    x264_macroblock_cache_mv_ptr_tpc( 0, 0, 2, 4, 0, a->l0.me8x16[0].mv, mb );
                    x264_macroblock_cache_mv_ptr_tpc( 2, 0, 2, 4, 0, a->l0.me8x16[1].mv, mb );
                    break;
        ...
        case P_8x8:
            x264_macroblock_cache_ref_tpc( 0, 0, 2, 2, 0, a->l0.me8x8[0].i_ref, mb );
            x264_macroblock_cache_ref_tpc( 2, 0, 2, 2, 0, a->l0.me8x8[1].i_ref, mb );
            x264_macroblock_cache_ref_tpc( 0, 2, 2, 2, 0, a->l0.me8x8[2].i_ref, mb );
            x264_macroblock_cache_ref_tpc( 2, 2, 2, 2, 0, a->l0.me8x8[3].i_ref, mb );
            for( i = 0; i < 4; i++ )
                x264_mb_cache_mv_p8x8_tpc(  a, i, mb );
            break;
        ...
        case B_SKIP:
        case B_DIRECT:
            x264_mb_load_mv_direct8x8_tpc( 0, mb );
            x264_mb_load_mv_direct8x8_tpc( 1, mb);
            x264_mb_load_mv_direct8x8_tpc( 2, mb );
            x264_mb_load_mv_direct8x8_tpc( 3, mb );
            break;

        case B_8x8:
            /* optimize: cache might not need to be rewritten */
            for( i = 0; i < 4; i++ )
                x264_mb_cache_mv_b8x8_tpc( h, a, i, 1, mb );
            break;
        ...
        default: /* the rest of the B types */
            ...
            case D_16x16:
                ...
                break;
            case D_16x8:
                x264_mb_cache_mv_b16x8_tpc( a, 0, 1, mb );
                x264_mb_cache_mv_b16x8_tpc( a, 1, 1, mb );
                break;
            case D_8x16:
                x264_mb_cache_mv_b8x16_tpc( a, 0, 1, mb );
                x264_mb_cache_mv_b8x16_tpc( a, 1, 1, mb );
                break;
            default:
                x264_log( h, X264_LOG_ERROR, "internal error (invalid MB type)\n" );
                break;
...
#ifndef NDEBUG
    if( h->param.i_threads > 1 && !IS_INTRA(mb->i_type) )
    {
        int l;
        for( l=0; l <= (h->sh.i_type == SLICE_TYPE_B); l++ )
        {
            ...
            if( ref < 0 )
                continue;
            completed = (l ? h->fref1 : h->fref0)[ ref >> mb->b_interlaced ]->i_lines_completed;
            if( (mb->cache.mv[l][x264_scan8[15]][1] >> (2 - mb->b_interlaced)) + mb->i_mb_y*16 > completed )
            {
                x264_log( h, X264_LOG_WARNING, "internal error (MV out of thread range)\n");
                fprintf(stderr, "mb type: %d \n", mb->i_type);
                fprintf(stderr, "mv: l%dr%d (%d,%d) \n", l, ref,
