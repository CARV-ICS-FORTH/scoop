...
int main( int argc, char **argv )
{
    ...
    if (MAX_ARGS<16){
    fprintf(stderr,"#############################################\n");
    fprintf(stderr,"ERROR: Number of args in TPC less than 16 !!!\n");
    fprintf(stderr,"#############################################\n");
	exit(-1);
    }

    if (MAX_QUEUE_ENTRIES>1){
    	fprintf(stderr,"************************************************************************************\n");
    	fprintf(stderr,"WARNING: Number of queue entries more than 1: be sure you know what are you doing!!!\n");
    	fprintf(stderr,"************************************************************************************\n");
    }
...
static int  Encode( x264_param_t *param, cli_opt_t *opt )
{
    ...
    char    buf[200];
    uint64_t time1, time2, time4, read_file_time=0;
    ...
    /* Create a new pic */
    x264_picture_alloc( Gpics[0], X264_CSP_I420, param->i_width, param->i_height );
    x264_picture_alloc( Gpics[1], X264_CSP_I420, param->i_width, param->i_height );
    Gpics[0]->alloc=0;
    Gpics[1]->alloc=0;
    Gfenc_next=NULL; 
...
#if 0
    memset( h->outstanding, 0x00, sizeof(uint16_t)*SAMPLES);
    h->pos = 0;
    h->last_time_sample = __mftb();
#endif
...
#ifdef TASK_STAT
        for ( i=0; i<h->spes; i++)
                tpc_call( 26,1 , &h->mb, sizeof(h->mb), TPC_IN_ARG );
#endif

	/* In our stats we don't want IO stats */
	time4 = __mftb();
	if (Gpics[(h->i_thread_phase) % 2]->alloc==0)
        if( p_read_frame( Gpics[(h->i_thread_phase ) % 2], opt->hin, i_frame + opt->i_seek ) )
            break;


        if( p_read_frame( Gpics[ (h->i_thread_phase + 1) % 2], opt->hin, i_frame + opt->i_seek ) )
		Gpics[ (h->i_thread_phase + 1) % 2]->alloc=0;
	else
		Gpics[ (h->i_thread_phase + 1) % 2]->alloc=1;
 
#ifdef TASK_STAT
        for ( i=0; i<h->spes; i++)
                tpc_call( 27,1 , &h->mb, sizeof(h->mb), TPC_IN_ARG );
#endif
	time4 =  __mftb() - time4; 
	read_file_time += time4 ;

#if 0
{
	uint16_t idle = time4/SAMPLE_TIME ;		\
	h->pos -= idle; 				\
 	h->pos++;
        h->outstanding[h->pos]=-1;		\
 	h->pos++;
}
#endif

#ifdef STAT
     stats[h->sh.i_type][FRAME_READ] += ( __mftb() - time3 );
#endif
        Gpics[(h->i_thread_phase ) % 2]->i_pts = (int64_t)i_frame * param->i_fps_den;

        if( opt->qpfile )
            parse_qpfile( opt, Gpics[(h->i_thread_phase + 1) % 2], i_frame + opt->i_seek );
        else
        {
            /* Do not force any parameters */
            Gpics[(h->i_thread_phase ) % 2]->i_type = X264_TYPE_AUTO;
            Gpics[(h->i_thread_phase ) % 2]->i_qpplus1 = 0;
        }

        i_file += Encode_frame( h, opt->hout, Gpics[(h->i_thread_phase ) % 2] );
    ...
    uint64_t total_time=time2-time1-read_file_time;



    tpc_print_stats(stdout);
    fprintf( stderr," Encode ticks: %llu \n",total_time );

    fprintf( stderr," IO_ticks: %llu \n", read_file_time);
#ifdef MEMORY_MANAGE
    fprintf( stderr," total_util: %d \n", 100*h->i_pool_count/h->mb.i_mb_count  );
#endif

    fprintf( stderr," Call ticks: %llu \n", x264_call_ticks );

#ifdef TIME_WRITE
    fprintf( stderr," write ticks: %llu\n", write_ticks );
#endif

    fprintf( stderr," Encode FPS: %lf \n", ((double) i_frame)/((double)total_time/79800000) );


    print_spe_stats();

    x264_picture_clean( Gpics[0] );
    x264_picture_clean( Gpics[1] );
    ...
    fprintf( stderr, "\n" );
     ...
     for ( ii=0; ii< MAX_STAT; ii++){
	/* don't print read file  stats */
	if ( (ii==FRAME_READ) ) continue;
        total_curr=0;

  	if (ii!=MANAGE) {      for ( jj=0; jj<MAX_FRAME_TYPE; jj++) total_curr+= stats[jj][ii]; }
	else {     	for ( jj=0; jj<MAX_FRAME_TYPE ; jj++) manage_stats+= stats[jj][ii] ;  }

       	if (ii!=MANAGE)
           fprintf(stderr," [%16s]: %11llu  --  %5.2lf ms -- normalized: %2.3lf  \n", 
                         func_name[ii], 
                         ( total_curr ),
                        (double)( total_curr)/((double)79800),
			 ((double) total_curr/ (double) (total_time) )*100
               );
    }   

     /* Print the "other" time */
     total_curr=0;
     for ( ii=0; ii< MAX_STAT; ii++){
	/* don't print read file  stats */
	if (ii==FRAME_READ || ii==MANAGE) continue;
       	for ( jj=0; jj<MAX_FRAME_TYPE ; jj++) total_curr+= stats[jj][ii] ;
     }

     manage_stats= manage_stats-total_curr;

      fprintf(stderr," [%16s]: %11llu  --  %6.2lf ms -- normalized: %2.3lf   \n", 
                       func_name[MAX_STAT],
                        ( total_time-total_curr-manage_stats),
                        (double)( total_time-total_curr-manage_stats)/((double)79800),
			 ((double) (  total_time-total_curr-manage_stats )/ (double) (total_time) )*100
          );

      fprintf(stderr," [%16s]: %11llu  --  %6.2lf ms -- normalized: %2.3lf  \n", 
                       func_name[MANAGE],
                         manage_stats,
                        (double)( manage_stats)/((double)79800),
			 ((double) manage_stats/ (double) (total_time) )*100
          );
