...
void x264_macroblock_call_analyse_tpc(  x264_t *h, x264_mb_t *mb, int mb_i_xy, x264_dct_t *dct, int i_mb_x, int i_mb_y  ){
    ...
    const int i_pix_offset_luma =  16 * (i_mb_x + i_mb_y * i_stride_dec_luma);

    const int i_pix_offset_chroma = 8 * (i_mb_x + i_mb_y * i_stride_dec_chroma);

    mb->i_qp = mb->i_last_qp = (h->mb.i_last_qp);                      
    mb->i_chroma_qp = (h->mb.i_chroma_qp) ;			
    ...
    int i_mb_threshold = 32/16 + i_me_range/16;
   ...
   uint64_t   tmp_time1 = __mftb();
    ...
    if ( h->sh.i_type == SLICE_TYPE_B){
	    ...
	    SET_ARGUMENT( entry, 0,    fdec_luma, TPC_BUILD_STRIDEARG(16,16),  TPC_STRIDE_ARG|TPC_OUT_ARG,  i_stride2_luma   );
	    SET_ARGUMENT( entry, 1, fdec_chroma1, TPC_BUILD_STRIDEARG( 8,16),  TPC_STRIDE_ARG|TPC_OUT_ARG,  i_stride2_chroma );
	    SET_ARGUMENT( entry, 2, fdec_chroma2, TPC_BUILD_STRIDEARG( 8,16),  TPC_STRIDE_ARG|TPC_OUT_ARG,  i_stride2_chroma );


	    SET_ARGUMENT( entry, 3,           mb,                sizeof(*mb),               TPC_INOUT_ARG,                 0 );
	    SET_ARGUMENT( entry, 4,          dct,               sizeof(*dct),               TPC_INOUT_ARG,                 0 );
	    SET_ARGUMENT( entry, 5,         data,              sizeof(*data),                  TPC_IN_ARG,                 0 );

            SET_ARGUMENT( entry, 6,         ref0, TPC_BUILD_STRIDEARG(luma_range_y,range),          TPC_STRIDE_ARG|TPC_IN_ARG, i_stride);
            SET_ARGUMENT( entry, 7,  chroma_ref0, TPC_BUILD_STRIDEARG(chroma_range_y,chroma_range), TPC_STRIDE_ARG|TPC_IN_ARG, chroma_i_stride0);
    	    SET_ARGUMENT( entry, 8,  chroma_ref1, TPC_BUILD_STRIDEARG(chroma_range_y,chroma_range), TPC_STRIDE_ARG|TPC_IN_ARG, chroma_i_stride1);

    	    SET_ARGUMENT( entry, 9,          ref1, TPC_BUILD_STRIDEARG(luma_range_y,range),          TPC_STRIDE_ARG|TPC_IN_ARG, i_stride);
       	    SET_ARGUMENT( entry, 10,  chroma_ref2, TPC_BUILD_STRIDEARG(chroma_range_y,chroma_range) ,TPC_STRIDE_ARG|TPC_IN_ARG, chroma_i_stride0);
       	    SET_ARGUMENT( entry, 11,  chroma_ref3, TPC_BUILD_STRIDEARG(chroma_range_y,chroma_range) ,     TPC_STRIDE_ARG|TPC_IN_ARG, chroma_i_stride1);

            SET_ARGUMENT( entry, 12,     fenc_luma,    TPC_BUILD_STRIDEARG(16,16), TPC_STRIDE_ARG|TPC_IN_ARG, i_stride2_luma);
	    SET_ARGUMENT( entry, 13,  fenc_chroma1,    TPC_BUILD_STRIDEARG(8,16), TPC_STRIDE_ARG|TPC_IN_ARG, i_stride2_chroma );
	    SET_ARGUMENT( entry, 14,  fenc_chroma2,    TPC_BUILD_STRIDEARG(8,16), TPC_STRIDE_ARG|TPC_IN_ARG, i_stride2_chroma );


	    h->tpc_id_table[mb_i_xy] = tpc_call_descriptor (1, 15, &entry);
    ...
    if ( mb->i_mb_y > ( i_height_mb - i_mb_threshold)  ){
	   ...
	   chroma_range_y = chroma_range - (8)*( -1 + i_mb_threshold - (i_height_mb - mb->i_mb_y)); 
	    ...
	    SET_ARGUMENT( entry, 5,         data,              sizeof(*data),                  TPC_IN_ARG,                 0 );
            ...
            SET_ARGUMENT( entry, 9,     fenc_luma,    TPC_BUILD_STRIDEARG(16,16), TPC_STRIDE_ARG|TPC_IN_ARG, i_stride2_luma);
	    SET_ARGUMENT( entry, 10,  fenc_chroma1,    TPC_BUILD_STRIDEARG(8,16), TPC_STRIDE_ARG|TPC_IN_ARG, i_stride2_chroma );
	    SET_ARGUMENT( entry, 11,  fenc_chroma2,    TPC_BUILD_STRIDEARG(8,16), TPC_STRIDE_ARG|TPC_IN_ARG, i_stride2_chroma );



	    h->tpc_id_table[mb_i_xy] = tpc_call_descriptor (1, 12, &entry);
...
void x264_macroblock_call_me_tpc(  x264_t *h, x264_mb_t *mb, int mb_i_xy, x264_dct_t *dct, int i_mb_x, int i_mb_y  ){
...
//#define SET_ARGUMENT( task, arg_i, arg_ea, arg_size, arg_type, arg_stride )  
    ...
    if ( h->sh.i_type == SLICE_TYPE_B){
	    ...
	    SET_ARGUMENT( entry, 3,           mb,                sizeof(*mb),               TPC_INOUT_ARG,                 0 );
	    SET_ARGUMENT( entry, 4,          dct,               sizeof(*dct),               TPC_IN_ARG,                 0 );
	    SET_ARGUMENT( entry, 5,         data,              sizeof(*data),                  TPC_IN_ARG,                 0 );
	    ...
	    h->tpc_id_table[mb_i_xy] = tpc_call_descriptor (29, 15, &entry);
    ...
    if ( mb->i_mb_y > ( i_height_mb - i_mb_threshold)  ){
	   ...
	   chroma_range_y = chroma_range - (8)*( -1 + i_mb_threshold - (i_height_mb - mb->i_mb_y)); 
	    ...
	    h->tpc_id_table[mb_i_xy] = tpc_call_descriptor (29, 12, &entry);
