...
typedef struct {
    ...
    int next_frame;
    int idx;
} __attribute__ ((aligned (16)))  yuv_input_t;
...
static inline void cache_load_tpc(x264_data_t *data, x264_mb_t *mb){

	/* Non zero count */
	const int i_top_xy = mb->i_mb_x + (mb->i_mb_y-1)*mb->i_mb_stride;
	const int i_mb_x = mb->i_mb_x;
	const int i_mb_y = mb->i_mb_y;
	const int i_mb_xy = mb->i_mb_xy;

	const int i_mb_4x4 = 4*(i_mb_y * mb->i_b4_stride + i_mb_x);
	const int i_mb_8x8 = 2*(i_mb_y * mb->i_b8_stride + i_mb_x);
	...
	if( i_top_xy >= 0 )
	{
		...
		mb->i_neighbour |= MB_TOP;
	}

	if( i_mb_x > 0 && i_mb_xy > 0 )
	{

		/* TODO: This also can executed in SPE */
		mb->i_neighbour |= MB_LEFT;

	}
	if( i_mb_x < mb->i_mb_stride - 1 && i_top_xy + 1 >= 0 )
	{
		mb->i_neighbour |= MB_TOPRIGHT;
	}
	else
		mb->i_mb_type_topright = -1;
	if( i_mb_x > 0 && i_top_xy - 1 >= 0 )
	{
		mb->i_neighbour |= MB_TOPLEFT;
	}
	else
		mb->i_mb_type_topleft = -1;
...
int execute_task( queue_entry_t *ex_task, tpc_spe_task_state_t *task_info)
{
...
#ifdef TASK_STAT
    uint32_t count=0;
    static uint32_t stat_issued[4096]={0};
    static uint16_t stat_diff[4096]={0};
    static uint8_t stat_task_id[4096]={0};
    static int iter = 0;

    static uint32_t stat_IO_start[32]={0};
    static uint32_t stat_IO_end[32]={0};
    static uint32_t iter2=0;
    static uint32_t iter3=0;
    stat_issued[iter] = spu_read_decrementer();
#endif
    ...
    switch(ex_task->funcid)
    {
		...
		/* OUT ARGS */
	        ...
	        /* these are  fdec */
	        /* Fdec frame luma */
	        fdec0 =  (uint8_t *)((void *)   task_info->ls_addr );
	        arg13  =  (void *)((void *)   task_info->ls_addr );
		...
		/* DCT struct  */
	      	dct = (x264_dct_t *)((void *)arg1 + ex_task->arguments[3].size);
	      	arg2 = ((void *)arg1 + ex_task->arguments[3].size);

///////////////////////////// 


		/* Metadata */
		data = (x264_data_t *)(  (void *)arg2 + ex_task->arguments[4].size );
                arg3 = (void *) ((void *)arg2 + ex_task->arguments[4].size);

 

		/* Reference frame (1) */
  
                ref0 = (uint8_t *)((void *)arg3 + ex_task->arguments[5].size );
                arg4 = (void *) ((void *)arg3 + ex_task->arguments[5].size);
                elems = TPC_EXTRACT_STRIDEARG_ELEMS(ex_task->arguments[6].size);
                elem_sz = TPC_EXTRACT_STRIDEARG_ELEMSZ(ex_task->arguments[6].size);
	...
	case 12:/*  Copy intra */
		...
		/* Deblock chroma */
	        ...
	        elem_sz = TPC_EXTRACT_STRIDEARG_ELEMSZ(ex_task->arguments[0].size);
	        /* Fdec frame chroma */
	        fdec1 =  (uint8_t *)((void *)arg2 + elems*elem_sz );
	...
	case 24: 
		...
		break;
         ...
         case 25:
   		    ...
   		    puts(" ENCODE ");
                    for (i=0; i<iter;i++)
			if (stat_task_id[i]==1)
                            printf(" %u  %u \n",  (-stat_issued[i] + base)*40,  (stat_diff[i] )*40 );

   		    puts(" ENTROPY ");
                    for (i=0; i<iter;i++)
			if (stat_task_id[i]==10)
                            printf(" %u  %u \n",  (-stat_issued[i] + base)*40,  (stat_diff[i] )*40 );

   		    puts(" DEBLOCK ");
                    for (i=0; i<iter;i++)
			if (stat_task_id[i]==14 || stat_task_id[i]== 15)
                            printf(" %u  %u \n",  (-stat_issued[i] + base)*40,  (stat_diff[i] )*40 );

   		    puts(" OTHER ");
                    for (i=0; i<iter;i++)
			if (    (stat_task_id[i]!=14) && (stat_task_id[i]!= 15) &&  (stat_task_id[i]!= 10)  && (stat_task_id[i]!= 1) )
                            printf(" %u  %u \n",  (-stat_issued[i] + base)*40,  (stat_diff[i] )*40 );

                     puts(" -- ");
                     for (i=0;i<iter2;i++)
                           printf("File IO from %u to %u   \n", (-stat_IO_start[i]+base)*40, (-stat_IO_end[i]+base)*40 );
...
#ifdef TASK_STAT
         case 26:
                stat_IO_start[iter2++] = spu_read_decrementer();
                break;
         case 27:
                stat_IO_end[iter3++] = spu_read_decrementer();
                break;
#endif
		...
		/* DST */
             	arg1 = ( void *) task_info->ls_addr;
	      	h = ((void *) arg1 + ex_task->arguments[0].size);

		fseek( h->fh, (uint64_t) (h->next_frame) * h->width * h->height * 3 / 2, SEEK_SET ); 
		fread( arg1, 1, 256, h->fh );
...
#ifdef TASK_STAT
    stat_diff[iter] = (uint16_t) (stat_issued[iter] -spu_read_decrementer());
    stat_task_id[iter] = ex_task->funcid;
    iter++;
#endif
