...
FILE *dfopen(char *file, char *perm) {
...
int read_frame_yuv( x264_picture_t *p_pic, hnd_t handle, int i_frame )
{
    ...
    //dfread( p_pic->img.plane[0], 1, h->width * h->height, h->fh ) ;
      //      dfread( p_pic->img.plane[1], 1, h->width * h->height / 4, h->fh );
        //    dfread( p_pic->img.plane[2], 1, h->width * h->height / 4, h->fh ) ;
