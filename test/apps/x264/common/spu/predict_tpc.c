...
void predict_16x16_p( uint8_t *src )
{
    ...
    for( y = 0; y < 16; y++ )
    {
        ...
        for( x = 0; x < 16;  )
        {
            ...
            src[x+1] = x264_clip_uint8( pix>>5 );
            pix += b;

            src[x+2] = x264_clip_uint8( pix>>5 );
            pix += b;

            src[x+3] = x264_clip_uint8( pix>>5 );
            pix += b;
