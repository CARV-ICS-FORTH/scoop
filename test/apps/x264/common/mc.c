...
static void mc_copy( uint8_t *src, int i_src_stride, uint8_t *dst, int i_dst_stride, int i_width, int i_height )
{
    ...
    for( y = 0; y < i_height; y++ )
    {
	CACHE_PREFETCH (src+i_src_stride);
...
//#ifdef TPC
	...
	x264_macroblock_call_hpel_filter(h, frame->filtered[1] + offs,
				frame->filtered[2] + offs,
				frame->filtered[3] + offs,
				frame->plane[0] + offs,
				stride, width + 16, (height - start)/4, mb_y );

	x264_macroblock_call_hpel_filter(h, frame->filtered[1] + offs,
				frame->filtered[2] + offs + (height - start)/4 ,
				frame->filtered[3] + offs + (height - start)/4,
				frame->plane[0] + offs,
				stride, width + 16 , (height - start)/4, mb_y );


	x264_macroblock_call_hpel_filter(h, frame->filtered[1] + offs,
				frame->filtered[2] + offs + 2*(height - start)/4 ,
				frame->filtered[3] + offs + 2*(height - start)/4,
				frame->plane[0] + offs,
				stride, width + 16 , (height - start)/4, mb_y );

	x264_macroblock_call_hpel_filter(h, frame->filtered[1] + offs,
				frame->filtered[2] + offs + 3*(height - start)/4 ,
				frame->filtered[3] + offs + 3*(height - start)/4,
				frame->plane[0] + offs,
				stride, width + 16 , (height - start)/4, mb_y );
