...
void x264_visualize_mb_tpc( x264_t *h, x264_mb_t *mb )
{
    ...
    v->i_type = mb->i_type;
    v->i_partition = mb->i_partition;
    for (i=0; i<4; i++) v->i_sub_partition[i] = mb->i_sub_partition[i];
    for (y=0; y<4; y++) for (x=0; x<4; x++)
        v->intra4x4_pred_mode[y][x] = mb->cache.intra4x4_pred_mode[X264_SCAN8_0+y*8+x];
    for (l=0; l<2; l++) for (y=0; y<4; y++) for (x=0; x<4; x++) {
        for (i=0; i<2; i++) {
            v->mv[l][y][x][i] = mb->cache.mv[l][X264_SCAN8_0+y*8+x][i];
        }
        v->ref[l][y][x] = mb->cache.ref[l][X264_SCAN8_0+y*8+x];
    }
    v->i_intra16x16_pred_mode = mb->i_intra16x16_pred_mode;
}
...
void x264_visualize_show_tpc( x264_t *h, x264_mb_t *mb, int spu_id )
{
    int mb_xy;
    static const stringlist_t mb_types[] = {
        /* Block types marked as NULL will not be drawn */
  ...
  static const char * mb_spu[] = {
    ...
    {
        const int mb_y = mb->i_mb_xy / h->sps->i_mb_width;
        const int mb_x = mb->i_mb_xy % h->sps->i_mb_width;
        char *const col =  mb_spu[spu_id];
//GET_STRING(mb_types, mb->i_type);
        int x = mb_x*16*zoom;
        int y = mb_y*16*zoom;
        int l = 0;
        unsigned int i, j;
