...
// c: IN/OUT
// a: IN
void spe_bmodd(double *c, double *a, struct bmodd_args *args)
{
  int dimi=args->dimi;
  int dimj=args->dimj;
  int stride_a=args->stride_a;
...
// a: IN
// b: IN
void spe_bmod(double *c, double *a, double *b, struct bmod_args *args)
{
  int dimi=args->dimi;
  int dimj=args->dimj;
  int dimk=args->dimk;
...

{
  ...
  last_malloc = (double **) tpc_malloc(P*sizeof(double *));;
  for (i=0;i<P;i++) {
    proc_bytes[i] = 0;
    last_malloc[i] = NULL;
  }
  ...
  for (i=0;i<P;i++) {
    last_malloc[i] = (double *) tpc_malloc(proc_bytes[i] + PAGE_SIZE);
...
/* Note that this causes all blocks to start out page-aligned, and that
  ...
  a = (double **) tpc_malloc(nblocks*nblocks*sizeof(double *));;
  if (a == NULL) {
  ...
  rhs = (double *) tpc_malloc(n*sizeof(double));;
  if (rhs == NULL) {
...

{
  ...
  int argsize = bs*bs*sizeof(double);
  //printf("argsize=%d\n", argsize);
  ...
  #pragma tpc(A(inout, argsize), D(in, argsize), bdivargs(in, ssize))
  spe_bdiv(A, D, bdivargs);
// 	tpc_call(0, 3,
...
// 	    bdivargs, sizeof(struct bdiv_args), TPC_IN_ARG);
  ...
  ssize = sizeof(struct bmodd_args);
  ...
  #pragma tpc(A(inout, argsize), D(in, argsize), bmoddargs(in, ssize))
  spe_bmodd(A, D, bmoddargs);
// 	tpc_call(1, 3,
// 	    A, argsize, TPC_INOUT_ARG,
// 	    D, argsize, TPC_IN_ARG,
// 	    bmoddargs, sizeof(struct bmodd_args), TPC_IN_ARG);
  ...
  ssize = sizeof(struct bmod_args);
  
	  //bmod(A, B, C, strI, strJ, strK, strI, strK, strI);
    #pragma tpc(C(inout, argsize), A(in, argsize), B(in, argsize), bmodargs(in, ssize))
    spe_bmod(C, A, B, bmodargs);
// 	  tpc_call(2, 4,
...

{
  ...
  edge = n%block_size;
  y = (double *) tpc_malloc(n*sizeof(double));  
  if (y == NULL) {
